...
- Reading layer (how an opened script becomes sentences): one grammar for typed and picked rows in
  `res://addons/eventsheet/editor/interaction/sentence_grammar.gd` (`EventSheetSentence`), with
  `viewport_lenses.gd`, `viewport_reading_rows.gd` and `viewport_row_builder.gd` beside it. The
  user-facing words are the sheet's own - event / condition / action / expression / function /
  trigger / object / behavior / family / layout / Script block / collapse / expand / Object bar /
  Object properties / Manual. "verb", "code card" and "fold" are not user-facing words (identifiers
  may keep `verb_*`).
- The Manual (the in-editor documentation reader): `res://addons/eventsheet/editor/doc/`, reading the
  shipped bundle in `res://addons/eventsheet/help/` - a verbatim copy of `docs/` regenerated by
  `tools/build_help_bundle.gd` and byte-gated by `tests/doc_library_test.gd`. Never hand-edit the
  bundle.
- Completion (what a field offers as you type): `res://addons/eventsheet/editor/autocomplete/` -
  `EventSheetCompletions` is the ONE list of candidates (keyed by field kind, which is the
  parameter's own `hint`, cached per sheet and only filtered per keystroke), `EventSheetCompletionPopup`
  the ONE popup riding a `LineEdit`, and `EventSheetQuickAdd` the reading of a whole typed sentence
  in the Add picker. Public through `EventSheets.completions_for` / `register_completion_source` /
  `attach_completions`. A code box (a Script block, the ƒx expression field) keeps Godot's own
  completion popup and is FED from the same seam - there is no second completion engine.
...
- `docs/README.md` - the documentation index: every guide and reference grouped by task. It is the
  authoritative list (a suite check fails if a `docs/*.md` is missing from it), so add a new guide
  there rather than lengthening this map. The entries below are only the ones an agent reaches for
  most often.
...
- `docs/GUIDE-USING-THE-ACE-STUDIO.md` - the ACE Studio / "New Function" dialog (the action / condition / expression kind cards, live preview, Ships-as, publish; parameters are edited from the function row's cells, guards are condition rows in the body)
- `docs/GUIDE-USING-WITH-EXISTING-CODE.md` - what an opened `.gd` (or `.tscn`) reads like, and how sheets call and are called by hand-written code
- `docs/GUIDE-BLOCK-STYLES.md` - the field guide to every block kind and the icon legend
- `docs/GUIDE-EDITOR-TOOLS.md` - the Editor Tool / Editor Plugin / Import Tool / Export Hook sheet family
- `docs/GUIDE-WRITING-THE-DOCS.md` - how this documentation set is built, and the regenerate-before-commit gate
