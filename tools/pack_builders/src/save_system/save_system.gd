# Pack source - save_system. The behaviour code this pack ships, as real GDScript: highlighted,
...
func set_slot_detail(detail_name: String, value) -> void:
	var data: Dictionary = _read_all()
	if not _last_read_ok:
		_fail_save(slot, "slot %d exists but could not be read - refusing to overwrite it (%s)." % [slot, _last_read_problem])
		return
...
func slot_detail(slot_index: int, detail_name: String, fallback) -> Variant:
...
func save_key_is(key: String, value) -> bool:
