# Pack source - post_kit. The behaviour the pack ships, as real GDScript: highlighted, parse-checked
# and breakpointable here, and assembled into the pack by Lib.pack_from_source. The #region below is
# one piece of the sheet; the two functions under it are the bodies of its _ready and _process rows.
extends Node
