...
#
# WHY EVERY FIELD BELOW IS `@ace_hidden`, AND THE STYLE PUBLISHES NO ROW AT ALL. A style is EDITED IN
# THE INSPECTOR and put in force by the shape's own `Apply Shape Style`. A reflected row per field
# here would be a row that writes a style the compiler makes on the spot and nothing wears - it
# compiles, it runs, and no pixel moves - while duplicating by name the rows the shape base already
# publishes (`Set Thickness`, `Set Dashes`, `Set Shape Colour`), so the picker would answer "how do I
# thicken this" with two entries, one of which does nothing. Changing a look at run time is the
# SHAPE's sentence, not the file's. A style that has to be built in code is `ShapeStyle.new()`
# through Set Property like any other resource.
extends Resource
