...
static func build() -> bool:
	...
	Lib.append_function(sheet, "use_advanced_random", "Use Advanced Random", "Loot", "When on, rolls draw from the shared AdvancedRandom autoload instead of this pack's own generator, so one seed drives your whole game's randomness. When off (the default) it uses its own seed. Needs the Advanced Random pack installed (it safely falls back to the local generator if not).",
		[["enabled", "bool"]],
		"_use_shared = enabled")
