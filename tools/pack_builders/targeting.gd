# Pack builder - targeting (one pack per file; run via tools/build_sample_behaviors.gd).
@tool

const Lib := preload("res://tools/pack_builders/_lib.gd")


## Targeting: the lock-on a twin-stick or top-down game asks for, and the aim help the
## accessibility rows already declare a radius for.
##
## The pack reads Engine's "aim_assist_radius" meta - the one the shipped Set Aim Assist Radius
## row writes and the Aim Assist Radius expression reads - so an options screen that already has
## the slider controls Assisted Aim and Magnetism without a line of new wiring, and a radius of
## zero turns both off.
static func build() -> bool:
	var src: Lib.PackSource = Lib.pack_from_source("targeting", "Node2D", "TargetingBehavior",
		...
		Lib.manifest().behavior().category("Targeting").tags(["combat", "aim", "targeting"]))
	src.note("Targeting behavior: attach it under the node that aims. Lock On To Nearest searches a cone around the host's facing for the closest member of a group; Cycle Target steps along that same cone, left to right, and wraps. A lock ends when the target dies, leaves the range, steps behind a wall (with sight checking on) or is released, and On Target Lost says which. Assisted Aim and Magnetism read the shipped Aim Assist Radius setting, so a zero radius is the off switch the options screen already has. This pack is an event sheet - extend it by editing it.")
	src.block("block_1")
	src.on_ready()
	src.on_process()
	src.verb("lock_nearest", "Lock On To Nearest",
		"Searches a cone around the host's facing for the closest member of a group inside a range, and holds it. On Target Locked fires when the held target changes; a search that finds nothing leaves the current lock alone, so a row polled every frame does not drop the target on the first frame it goes behind cover. Leave the group empty for the behaviour's own Target Group, and write 0 for the cone or the range to use its own defaults.",
	...
	src.verb("cycle_target", "Cycle Target",
		"Steps to the next candidate the last Lock On To Nearest found, left to right by angle, and wraps round to the leftmost after the rightmost. Candidates that died since the search are dropped first, so cycling never lands on a corpse. With nothing held it takes the leftmost.",
		[])
	...
	src.object_expression("locked_target", "Locked Target",
		"The node being held, or null when nothing is. Hand it to any row that takes a node: a homing bullet, a Look At, a damage number's parent.",
		[], "Node2D")
	src.expression("locked_target_on_screen", "Locked Target On Screen",
		"Where the held target sits on screen right now, camera zoom and scroll included - the position for a reticle living on a CanvasLayer. Vector2.ZERO when nothing is held.",
		[], TYPE_VECTOR2)
	src.expression("distance_to_target", "Distance To Target",
		"How far the held target is, in pixels. INF when nothing is held, so a row asking whether the target is closer than something is simply false rather than accidentally true.",
		[], TYPE_FLOAT)
	src.expression("assisted_aim", "Assisted Aim",
		...
		[["direction", "Vector2"], ["strength", "float"]], TYPE_VECTOR2)
	...
	return Lib.publish(src, "res://eventsheet_addons/targeting/targeting_behavior")
