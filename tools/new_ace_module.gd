...
static func _skeleton(base: String) -> String:
	...
	var lines: PackedStringArray = PackedStringArray([
		...
		"",
		"\t# An ACTION: does something. The maker is named after the kind of row it makes, and",
		"\t# its third argument is the GDScript it bakes to, with {param} slots.",
		"\tdescriptors.append(F.act(\"%sExample\", \"%s Example\", \"print({message})\", CAT, \"%s example {message}\", \"An example action - replace this with your own.\").param(\"message\", \"\\\"hello\\\"\", \"Message\", \"What to print.\", \"expression\"))" % [pascal, category, category.to_lower()],
		"",
		"\t# A CONDITION: returns true/false.",
		"\tdescriptors.append(F.cond(\"%sExampleCheck\", \"%s Example Check\", \"{value} > 0\", CAT, \"%s {value} is positive\", \"An example condition - replace this with your own.\").param_typed(\"float\", \"value\", \"1.0\", \"Value\", \"Number to test.\", \"expression\"))" % [pascal, category, category.to_lower()],
		"",
		"\t# An EXPRESSION: returns a value you can use in another field.",
		"\tdescriptors.append(F.expr(\"%sExampleValue\", \"%s Example Value\", \"absf({value})\", CAT, \"%s absolute {value}\", \"An example expression - replace this with your own.\").param_typed(\"float\", \"value\", \"-1.0\", \"Value\", \"Number to read.\", \"expression\"))" % [pascal, category, category.to_lower()],
		"",
		"\treturn descriptors",
		"",
		"",
