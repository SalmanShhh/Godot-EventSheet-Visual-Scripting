...
## Every path this run WROTE, filled by the three write helpers below on every run, check or not.
##
## THE GATE CANNOT ASK THE TREE THIS. A builder that no longer writes a showcase does not delete the
## file it used to write, so reading the tree back after the build finds the stale file sitting there
## and compares it, byte for byte, against itself - and passes. That drift kind was named in the
## gate's own output and could not fire. What the builder wrote is a fact only the builder has.
...
## The sidecars Godot's importer writes beside a file and nobody commits. They are not builder
## output and they are not in the repository, so the gate neither compares them nor asks the builder
## to account for them - a `.uid` regenerated by an import is not showcase drift.
const SIDECARS: PackedStringArray = ["uid", "import"]


## Every GENERATED file under the showcase tree as {res:// path: bytes}, walked in SORTED order so
## the gate names its drift in the same order on every filesystem - a directory-order walk reads like
## two different failures on Linux and on Windows for one identical tree.
func _read_tree() -> Dictionary:
	var found: Dictionary = {}
	for path: String in _tree_paths(SHOWCASE_ROOT):
		if SIDECARS.has(path.get_extension()):
			continue
...
## What this run WROTE against what was committed, then the committed bytes put back. Three kinds of
## drift are named separately, because the fix differs: a file that changed, a file the builder no
## longer writes, and a file the builder writes that nobody committed. Returns how many files
## drifted, and prints the verdict line the gate is read by.
##
## THE SECOND KIND IS ANSWERED BY `_written`, NOT BY THE TREE. A stale file the builder retired is
## still on disk after the build, so reading the tree back finds it and compares it against itself
## for ever. What the builder wrote is the only place that fact lives.
func _compare_and_restore() -> int:
	var rebuilt: Dictionary = _read_tree()
	var paths: PackedStringArray = PackedStringArray()
	for path: Variant in _committed.keys():
		paths.append(str(path))
	for path: Variant in _written.keys():
	...
	for path: String in paths:
		...
		elif committed != PackedByteArray(rebuilt.get(path, PackedByteArray())):
...
## One text file this builder owns, written and RECORDED. Every write of a generated showcase file
## goes through here, `_save_scene` or `_save_resource`, which is what lets the gate tell a file the
## builder no longer writes from a file it wrote unchanged.
func _write_text(path: String, text: String) -> bool:
	_written[path] = true
	DirAccess.make_dir_recursive_absolute(path.get_base_dir())
	var file: FileAccess = FileAccess.open(path, FileAccess.WRITE)
	if file == null:
		push_error("[build_examples] could not write %s" % path)
		return false
...
## One resource this builder owns, saved and RECORDED.
...
## THE EMITTED SHEET RE-SPACED to the style gate's two blank lines around functions. The compiler
## emits single-blank output by design and a showcase `.gd` is ordinary project code that the gate
## reads, so eight showcases each spelled the same five lines. The importer preserves blank lines, so
## the byte round-trip the showcase test pins still holds.
func _respace_functions(path: String) -> bool:
	var emitted: String = FileAccess.get_file_as_string(path)
	emitted = emitted.replace("\n\nfunc ", "\n\n\nfunc ")
	emitted = emitted.replace("\n\n## @ace_hidden\nfunc ", "\n\n\n## @ace_hidden\nfunc ")
	return _write_text(path, emitted)
...
func _build_uhtn_planning() -> bool:
	...
	if not _save_resource(plan, "res://demo/showcase/uhtn_planning/guard_plan.tres"):
		return false
...
func _build_enemy_stats() -> bool:
	...
	emitted = emitted.replace("\n\nfunc roll_damage", "\n\n\nfunc roll_damage")
	_write_text("res://demo/showcase/enemy_stats/enemy_stats.gd", emitted)
	...
	var saved: bool = _save_resource(stats, "res://demo/showcase/enemy_stats/enemy_stats_example.tres")
	print("[build_examples] enemy_stats_example.tres saved=%s" % str(saved))
	return saved
...
func _build_menu_starter() -> bool:
	...
	_respace_functions("res://demo/showcase/menu_starter/menu_starter.gd")
...
func _build_input_rebind() -> bool:
	...
	if not _compile(sheet, "res://demo/showcase/input_rebind/input_rebind.tres", "res://demo/showcase/input_rebind/input_rebind.gd"):
		return false
	_respace_functions("res://demo/showcase/input_rebind/input_rebind.gd")
...
func _build_path_chase() -> bool:
	...
	if not _compile(sheet, "res://demo/showcase/path_chase/path_chase.tres", "res://demo/showcase/path_chase/path_chase.gd"):
		return false
	_respace_functions("res://demo/showcase/path_chase/path_chase.gd")
...
func _build_draw_lab() -> bool:
	...
	if not _save_resource(prefab, "res://demo/showcase/draw_lab/target_marker.tres"):
	...
	if not _compile(sheet, "res://demo/showcase/draw_lab/draw_lab.tres", "res://demo/showcase/draw_lab/draw_lab.gd"):
		return false
	_respace_functions("res://demo/showcase/draw_lab/draw_lab.gd")
...
func _build_raycast_lab() -> bool:
	...
	if not _compile(sheet, "%s/raycast_lab.tres" % RAYCAST_LAB_DIR, "%s/raycast_lab.gd" % RAYCAST_LAB_DIR):
		return false
	_respace_functions("%s/raycast_lab.gd" % RAYCAST_LAB_DIR)
...
func _build_raycast_lab_3d() -> bool:
	...
	if not _compile(sheet, "%s/raycast_lab_3d.tres" % RAYCAST_LAB_3D_DIR, "%s/raycast_lab_3d.gd" % RAYCAST_LAB_3D_DIR):
		return false
	_respace_functions("%s/raycast_lab_3d.gd" % RAYCAST_LAB_3D_DIR)
...
func _build_hierarchy_playground() -> bool:
	...
	if not _compile(sheet, "%s/hierarchy_playground.tres" % HIERARCHY_DIR,
		...
		return false
	_respace_functions("%s/hierarchy_playground.gd" % HIERARCHY_DIR)
...
func _build_skill_tree() -> bool:
	...
	if not _save_resource(tree, "res://demo/showcase/skill_tree/adventurer_tree.tres"):
		return false
...
func _build_skate_park() -> bool:
	...
	if not _compile(sheet, "%s/skate_park.tres" % SKATE_PARK_DIR, "%s/skate_park.gd" % SKATE_PARK_DIR):
		return false
	_respace_functions("%s/skate_park.gd" % SKATE_PARK_DIR)
...
func _build_skate_park_3d() -> bool:
	...
	if not _compile(sheet, "%s/skate_park_3d.tres" % SKATE_PARK_3D_DIR,
		...
		return false
	_respace_functions("%s/skate_park_3d.gd" % SKATE_PARK_3D_DIR)
