...
The latest tagged release, **`v0.16.0` - "Open Anything, Publish Anything & Ask What Is There"**, is about the round trip - opening a script you already have as editable events, publishing a script you already own as verbs, and asking what is out there:
...
- **Everything reads as an event row**: a pack's published verbs were a flat spec table and its triggers a
  bare `signal …` strip - both now use the same two lanes as the rest of the sheet. A verb shows its kind
  badge, name and a clickable cell per input on the left, and what it hands back on the right, with the
  `@ace_description` you wrote as a caption above it and its body open beneath as ordinary sub-events; a
  trigger shows its published name and typed values on the left and the real signal it `emits` on the
  right. Verbs are spliced in the file's own order, so an opened pack reads top-to-bottom like its
  GDScript. Clicking a parameter opens a small **Edit Parameter** dialog rather than the whole verb form,
...
- **Your own script's verbs, without hand-writing annotations**: the ACE wizard's preview table
  (Sheet ▸ Custom Actions…) is now an EDITING surface - Publish, Kind, Verb and Category are editable
  in place, **Curate Script…** writes the differences back into your script as `## @ace_*` comments
  (backed up first, re-applying is a no-op, and only `##` comment lines ever move - no signature and
  no body is touched), **Parameters…** shapes a verb's hints, choices and starting values, and
  **Keep Old Name…** appends a deprecated forwarding stand-in so a RENAMED verb keeps working for the
  sheets that already use it. `@ace_param` gained a `default:`, `value=Label` option lists, and
  `hint: comparison` (the whole `=` / `!=` / `<` / `<=` / `>` / `>=` dropdown in one word), so an
  addon author can shape a param as well as a bundled module can. The **Project Doctor** gained an
  **orphaned-verb** check - a call to a provider member that no longer exists is the one failure here
  that compiles green and breaks at game runtime. And **dragging a parameter's label sideways scrubs
  its number** (Shift for a fine pass, Ctrl for a coarse one).
...
| _Unreleased on `main`_ - **Adopting code you already have.** Opening a hand-written `.gd` was measured at **88.1% verbatim code blocks**; it is now **zero** (0 of 25,974 lines across 206 real files), byte-exact on every one and gated by a test that opens REAL sources rather than fixtures. Four spelling-level bugs each reverted a whole file (the style guide's two blank lines between functions, an explicit `: Variant` parameter, a typed-collection return, a `#` note above a function), then the residue turned out to be structure nobody had split: **one statement is one action**, with comment runs as real comment rows, collection literals as **Declare** rows (body and file scope) whose entries edit in place and through an Add/Edit/Remove Entry menu, and split rules that never cut inside a class body, a string, or an open bracket. A compiler bug that **injected code into files whose comments merely discussed a convention** is fixed. **Your own classes are vocabulary with zero setup** - a **Your Project** picker section, right-click rename/recategorize/hide stored outside your script (or baked in as `## @ace_*` on request), "convert this raw call to the verb it matches", and a Doctor that names the member you probably renamed to; an adversarial review of that stack found 21 defects in green-suite code, all fixed. Plus **C3-style bold parameter emphasis** across the bundled vocabulary and every pack's featured verbs (l10n-safe), the inline colour picker's **saved palette**, **Ghost Row Part II** (suggestion chips + learn-as-you-type ranking), per-parameter **recent values**, a sticky **group breadcrumb**, **Compact Rows**, the **Picker Preview** rail, **Sheet ▸ Publish New Version**, the guide scaffolder, node-drop onto the Sheet Type dialog, the **Self section** (all four tiers, including live grounding to the running game), and the real macOS HiDPI fix. On top of that, rows now **read like sentences** (`Add wave[1] to score`, calls as Object then Verb, a **Reading Mode** lens, **Sheet > Name Raw Calls**), and the classic game patterns became rows: **Has Changed**, **named cooldowns**, **Remember Between Runs** (one toggle persists a variable), empty-safe group picking, and the State Machine pack telling **Time In State**; a second wave added **Move Toward (smooth)** (frame-rate-independent damping, runtime-proven), **Toggle**, **As Clock Time**, **Every X To Y Seconds**, the 2D **distance / aiming / screen-wrap / bob** verbs, the **Timeline block** (a schedule as rows compiling to the await-chain you would have written), and the Object Pool's **reset() seam**; a third wave made game FEEL rows - **coyote time as a condition** (`is_on_floor() was true within 0.1s`), input **press buffering**, the **wave director** (`On Group Emptied`), decaying **knockback**, magnet pull, orbit, hold-to-charge, **i-frames wired into Health's own damage**, Progress/Percent Of, burst repeat, and **floating text**. The sheet also grew a reading layer for structure itself: **identity bars** (the enum state list, Class setup as an inheritance breadcrumb with a facts dropdown, Host binding with its class icon) stand apart from content rows; a lifted **match** shows a plain-words caption with each case as a `◆ State:` row and each transition as a **nested condition row** (guards humanize - "Can See Player" - with an ƒ badge marking computed checks); badges everywhere are **SVG marks rasterized at exact pixel size**; and beginner-style files (`:=` variables, untyped lifecycle headers) round-trip byte-exact like everything else | 🚧 in repo |
