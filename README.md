...
| _Unreleased on `main`_ - **The Self section**, all four tiers: C3's `Self.` reflex in the ƒx Expressions dictionary (variables, C3-named properties, functions), the **Behaviours** subgroup with robust `get_node_or_null` lookups for spawn-heavy sheets, grounding to the **selected node's** real children, and **live grounding** to the running game over the Live Values channel - a behaviour attached at runtime lists under its real runtime name. Plus the plugin's chrome now tracks the editor display scale (the real macOS HiDPI fix, suite-linted) | 🚧 in repo |
