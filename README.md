...
- **Since `v0.9.5` (unreleased).**
  ...
  - **Open Sheets panel.** A filterable list of **open and recently-closed sheets** in the workspace (left of the viewport) — one click switches or reopens a sheet; toggle from **View › Open Sheets Panel** or collapse to a strip, both persisted per-project.
  - **Compiler:** an **included** (library) sheet's events now run **before** the root's (shared setup initialises first); a **disabled** group leaves a `# (disabled group … N rows omitted)` breadcrumb instead of vanishing silently.
...
| `docs/` | Contract specs (GDScript pairing, inspector attributes, addon composition, progressive disclosure, includes, event-group round-trip) + guides (inspector drawers & export options, C3 migration, recipes, performance, MCP, glossary, uninstall) |
