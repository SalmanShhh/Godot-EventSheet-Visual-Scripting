...
1. Copy `addons/eventforge/` and `addons/eventsheet/` into your Godot **4.5+** project (tested through **4.7 stable**). Optional: `eventsheet_addons/` for the 68 behavior packs. Removal is clean - see [uninstall](docs/GUIDE-UNINSTALL.md).
...
The latest release, **`v0.14.0` - "The Pathfinding & Game-Feel Update"**, gives enemies a brain and games their feel:

- **Platformer Pathfinding (2D jump graphs)** - build a nav graph from your `TileMapLayer` once and `Find Path To`: stairs walk, gaps and ledges connect through jump arcs and fall drops sized to the **sibling PlatformerMovement's real physics**, **portals** link any two spots in one action, and the discipline kit ships too (ledge restriction for patrollers, strict/relaxed jump positioning, AI coyote time, variable jumping, a stuck watchdog, follow-mode auto-repath, and a shared per-tick path budget for crowds). The **Path Chase** showcase runs all of it live.
- **Nav Agent 3D** - Godot's navmesh behind **the same verb names**: an auto-inserted `NavigationAgent3D`, runtime rebaking, RVO avoidance as a toggle, and the FPS Arena's stalker as proof.
- **The universal AI drive seam** - PlatformerMovement, 8-Direction, and the FPS Controller all carry the same inert-by-default `ai_controlled` + intent seam, so any driver steers them with the pack's own feel; with no movement pack at all, the pathfinder's fallback driver moves the body itself.
- **FPS movement tech + camera juice** - crouch (capsule shrinks, ceiling-checked stand), crouch slide, wall ride, wall jump on the FPS Controller; Recoil / Head Bob / Jitter / Tilt in the 2D **Juice** pack; and the new **Juice 3D** pack (trauma shake, cosmetic weapon recoil, lean, FOV punch, aim-down-sights zoom) applied as additive offsets that never fight the controller.
- **The input overhaul** - a **live Input Map picker** on every action parameter (custom ACEs opt in with one hint), ~30 new Mouse/Keyboard/InputMap/Gamepad helper ACEs (one-step rebinding on all three devices, binding-as-text, restore defaults, mouse focus + deltas, 3D cursor rays), and the **Input Rebind** showcase - a working options-menu slice.
- **Preview behaviors in the editor** - select a node, run one Tools command, and its behaviors animate in the viewport (Inspector tweaks reshape the motion live), restored exactly on stop; packs opt in with one pure static.
- **Builder power** - twelve new extension seams in the `EventSheets` API, the **Designing user-friendly ACEs** craft guide, drop-in CSV editor translations (EFIGS bundled), a ~21x faster plugin boot, System ACE parity, and per-showcase demo folders. The suite is now 62 packs.

**Quality** - 4,900+ assertions, all green, CI-gated on every push; byte-exact golden round-trips guard the lossless rules, and the pathfinding showcases are live-verified (the chaser jumps the gap and takes the portal on camera). **Verified on Godot 4.7 stable.** Generated code never depends on the plugin, templates bake at apply-time, and output is performance-identical to hand-written GDScript - all test-enforced.

_Recent releases before this:_ **v0.13.0** (the incremental/idle kit, ECS-lite Systems, Advanced Random behind one seed, data-driven loadouts), **v0.12.0** (the Inspector Designer + eight drawers, the UI trio packs, a faster lazily-built editor), and **v0.11.0** (collapsible regions, the abstraction levers, localisation, the public `EventSheets` API). The milestones table below and [CHANGELOG.md](CHANGELOG.md) have the full history.
...
| `eventsheet_addons/` | Zero-config ACE addons + the 62 behavior packs |
