...
> [!NOTE]
> **Early.** Every feature ships with tests (20,217 CI-gated assertions across 710 test files, byte-exact round-trip gates, performance-parity contracts), but the project hasn't yet earned real-world mileage and may see sweeping changes between releases. Pin a release tag and report what you hit - issues are read and acted on.
...
Delete the plugin and this script still runs. The reverse works too: **open *any* `.gd` as a sheet** - the round-trip is lossless and byte-identical, so you edit visually or in Godot's script editor with the two in sync. And "opens as a sheet" means it *reads* as one: measured over the 863 hand-written files of this repo's own source, **every code line but nine arrives as structure** (functions, events, actions, notes, declarations - 861 of the 863 files at zero verbatim code blocks, 9 block lines across 254,748 lines), in the sheet's own words - lifecycle handlers as triggers, `_unhandled_input` branches as Keyboard / Mouse events, hand-written signal handlers with payload chips, `A if C else B` as a sub-event pair with Else, and every published function as a Function block. A statement the vocabulary does not match stays an honest single-statement row, never a mangled one, and a shape the compiler cannot reproduce byte-exactly is left as an editable Script block rather than reformatted behind your back.
...
1. Copy `addons/eventforge/` and `addons/eventsheet/` into your Godot **4.5+** project (tested through **4.7 stable**). Optional: `eventsheet_addons/` for the 113 behavior packs. Removal is clean - see [uninstall](docs/GUIDE-UNINSTALL.md).
...
- **1,827 native ACEs** (and 96 triggers) - Tween, Scene flow, Audio, sprites and cameras, lights and the world's atmosphere, Nav, Math and Random, Color, every kind of raycast in 2D and 3D, collisions said as who touches what (named layers, a group filter on the touch, the step something changed), Nodes, Project / File utilities, runtime signal wiring, UI, particles, AnimationTree, tilemaps, shaders (a material's dials picked off the `.gdshader` that declares them, not typed), physics joints, input rebinding, seeded procedural generation, ECS-lite Systems, plus a **Helpers** escape hatch so unmapped code still stays an editable row.
- **113 behavior packs**, all authored as event sheets - every one with an icon, 94 of them with starred hero actions and a 15+ use-case guide of their own, the other 19 companion data assets and loaders documented inside their partner's guide - movement and feel (Platformer + Pathfinding, 8-Direction, Car, FPS Controller, Juice 2D / 3D …), AI (State Machine, Line of Sight, UtilityBrain, HTN / UHTN), combat and stats, the idle kit, narrative and content (Storylet Weaver, Dialogue Kit, Quest, Encounter Timeline …), drawing and UI, light and time of day (Light Flicker, Light Pulse, Day/Night Cycle), shader effects (Hit Flash, Dissolve, Outline, Grayscale, Wave, Screen FX), and system packs (Save System, Event Bus, Named Scenes, Game Settings, Debug Overlay, Platform Info …). See [docs/Addons/](docs/Addons/README.md).
...
**The editor reads itself** - the plugin's own source opens as event sheets in its own repository: **863 files, 861 of them at zero script blocks (9 block lines in 186,771 rows), and 89% of those rows in the sheet's own words**. A rotating sample is round-tripped byte-exact and held under a measured per-group ceiling on every run (`tests/plugin_reads_itself_test.gd`), and Tools ▸ Project Doctor reports the same numbers per group.

**Quality** - 20,217 assertions across 710 test files, all green, CI-gated on every push; byte-exact golden round-trips guard the lossless rules; the save backend is pinned across all 18 seams and six formats; showcases are live-verified on camera. **Verified on Godot 4.7 stable.** Generated code never depends on the plugin, templates bake at apply time, and output is performance-identical to hand-written GDScript - all test-enforced.
...
| `eventsheet_addons/` | Zero-config ACE addons + the 113 behavior packs |
