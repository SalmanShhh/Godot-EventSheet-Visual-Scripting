...
| _Unreleased (since `v0.9.0`)_ — **first-class variables, authoring & legibility**: variable **`@export` badge** + **`@export_group`/`@export_subgroup`** + variable tooltips with **lossless, editable `.gd` round-trip**; the full **Tier 3 Inspector drawers** (progress bar / dial / swatches / texture / curve) + Vector2/Color/Texture2D/Curve variable types; **"New Behaviour Addon…"** scaffold + **inline hover descriptions on every ACE** + **ACE deprecation**; **BBCode in cell text + descriptions**, an **inline cell colour picker**, **drag a Scene node onto a param**, scope-pill removal | 🚧 in progress |
...
| `docs/` | Contract specs (GDScript pairing, inspector attributes, addon composition, progressive disclosure) + guides (C3 migration, recipes, performance, MCP, glossary, uninstall) |
