...
- **The Manual, inside the editor**: every shipped doc link pins to the released tag, and Tools > Manual / F1 / `?` open all three doc sets - the guides, the 77 addon guides and **36 new module guides over all 48 builtin modules** - as natively rendered pages with **figures that draw themselves from real rows**, plus a **reference page for every object and behavior** (built from the live vocabulary, with a friendly stub and a one-click **Write this guide** where a guide is missing), a **glossary for readers coming from another event-sheet editor**, breadcrumb / back / forward / Recent / bookmarks, **F1 = help for the selected item** with **Follow selection**, one search box over all of it (Ctrl+Enter adds the verb at the caret), plus **Explain This Row** and a guide to writing the docs.
