...
> [!NOTE]
> **Early.** Every feature ships with tests (20,412 CI-gated assertions across 717 test files, byte-exact round-trip gates, performance-parity contracts), but the project hasn't yet earned real-world mileage and may see sweeping changes between releases. Pin a release tag and report what you hit - issues are read and acted on.
...
Delete the plugin and this script still runs. The reverse works too: **open *any* `.gd` as a sheet** - the round-trip is lossless and byte-identical, so you edit visually or in Godot's script editor with the two in sync. And "opens as a sheet" means it *reads* as one: measured over the 886 hand-written files of this repo's own source, **every code line but nine arrives as structure** (functions, events, actions, notes, declarations - 884 of the 886 files at zero verbatim code blocks, 9 block lines across 258,850 lines), in the sheet's own words - lifecycle handlers as triggers, `_unhandled_input` branches as Keyboard / Mouse events, hand-written signal handlers with payload chips, `A if C else B` as a sub-event pair with Else, and every published function as a Function block. A statement the vocabulary does not match stays an honest single-statement row, never a mangled one, and a shape the compiler cannot reproduce byte-exactly is left as an editable Script block rather than reformatted behind your back.
...
- **Multiplayer is Godot's own, in sentences.** Host a game, Join a game, Leave the game, the seven events the connection fires, `@rpc` messages, who is the host and who owns an object - each row is exactly the `ENetMultiplayerPeer` / `MultiplayerAPI` call it names, and a networked project you already wrote opens as those rows without changing a byte.
...
- **1,846 native ACEs** (and 98 triggers) - Tween, Scene flow, Audio, sprites and cameras, lights and the world's atmosphere, Nav, Math and Random, Color, every kind of raycast in 2D and 3D, collisions said as who touches what (named layers, a group filter on the touch, the step something changed), Nodes, Project / File utilities, runtime signal wiring, UI, particles, AnimationTree, tilemaps, shaders (a material's dials picked off the `.gdshader` that declares them, not typed), physics joints, input rebinding, seeded procedural generation, ECS-lite Systems, plus a **Helpers** escape hatch so unmapped code still stays an editable row.
- **114 behavior packs**, all authored as event sheets - every one with an icon, 95 of them with starred hero actions and a 15+ use-case guide of their own, the other 19 companion data assets and loaders documented inside their partner's guide - movement and feel (Platformer + Pathfinding, 8-Direction, Car, FPS Controller, Juice 2D / 3D …), AI (State Machine, Line of Sight, UtilityBrain, HTN / UHTN), combat and stats, the idle kit, narrative and content (Storylet Weaver, Dialogue Kit, Quest, Encounter Timeline …), drawing and UI, light and time of day (Light Flicker, Light Pulse, Day/Night Cycle), shader effects (Hit Flash, Dissolve, Outline, Grayscale, Wave, Screen FX), and system packs (Save System, Event Bus, Named Scenes, Second View, Game Settings, Debug Overlay, Platform Info …). See [docs/Addons/](docs/Addons/README.md).
...
- **The Manual, inside the editor** - Tools > Manual / F1 = help for the selected item / `?` prefix: the guides, every addon and module guide, a reference page per object and behavior, a glossary for readers coming from another event-sheet editor, one search box (Ctrl+Enter adds the action at the caret), examples that draw themselves from real rows, Explain This Row, step-by-step **tutorials** you follow in the sheet you have open, and a **What's new** page.
...
- **The Manual** - docked, F1 = help for the selected item, reference pages, glossary, search, tutorials, What's new, and the optional **Ask** box that answers in rows rather than in code.
- **Playing together** - Godot's high-level multiplayer in the sheet's own words: hosting, joining and leaving, the seven events the connection fires, `@rpc` messages answered as four questions in words, a group that says who runs it (everyone / the host / the owner), the scene's own two nodes as verbs (spawn a scene on every peer, keep a value in step, decide who may see it), **Play as host + client** in one click, and the four silent networking mistakes said under the row they are about. A networked project written before any of it existed opens on the same rows, byte for byte.
- **Lighting a game** - the light is the OBJECT, not a field: **Torch ▸ Set brightness to 1.2**, with one word each for brightness, colour, reach, on/off and shadows in both dimensions and the code echo showing the property that light really has. Darkness reads as a percentage (the file still keeps the colour), the `WorldEnvironment` is the **World** object, and the sheet head says what the scene is lit by, whether any occluder can block the shadows it casts, and how many other scenes share its environment file. **Light Flicker**, **Light Pulse** and **Day/Night Cycle** are behaviors any light can wear, the Doctor knows the five ways a light silently does nothing, and a project you already lit opens on those rows byte for byte.
- **Visual effects** - the shader file names its own dials, so **Boss ▸ Set `effect.dissolve` to 0.7** picks a name the `.gdshader` really declares, where `material.set_shader_parameter(&"disolve", 1.0)` is a call Godot accepts and never acts on. The declaration also picks the field the value is typed in (a `hint_range` gives a slider with the shader's own ends, a `source_color` the colour editor), the head grows an **effect** band saying which file the node wears and who else wears it, and six packs ship the effects nobody should have to write twice (**Hit Flash, Dissolve, Outline, Grayscale, Wave**, and **Screen FX** for the full screen, whose Fade carries `await` so a scene transition is two rows in one event). Doctor ▸ Effects knows the five ways an effect silently does nothing, and a project that already has shader code opens on those rows byte for byte.
- **Collisions, said as who touches what** - the node family decides what you may ask, so the picker only offers a sheet the rows its own node can answer and the dialog says which family this is while you fill it in. Layers speak the project's own names (**Collide with Enemies**, **Be on layer Player**) while the file still gets `set_collision_mask_value(2, true)`, so renaming a layer renames every row about it and moves nothing. The touch takes a group as a PARAMETER - **On collision with `<Group>`** emits the `is_in_group` early return a person would have written as the handler's visible first line - and **On landed** / **On first overlap** are the step a standing state changed, carrying the memory and the comparison as ordinary rows. The head reads *sees Enemies, Walls · seen by Player · monitoring on* out of the `.tscn`, and Doctor ▸ Collisions knows the four ways a correct row never fires, two of them with a door that writes the one property through the editor's own undo. A collision script you already wrote opens on those rows byte for byte.
- **The API is the vocabulary, and curation is the polish** - wherever the sheet can work out what class a receiver is, an ordinary call reads as an object-verb row and an ordinary property write reads as object, property, value, with the member name it really has, its parameter names and its own description taken off the class reference or your `##` lines. A derived row is drawn in a plainer call style with the class muted beside the object, so it is never mistaken for a curated sentence; a curated table landing later upgrades those rows in place with the file untouched. Four hand-written idioms gained names on the way (the input handler asking its event, a property that names its accessors, a connect that binds values, and the four notifications a game reacts to), a pack can teach the reader the lines its own verbs are written as, and what neither layer claims stays honest code, counted out loud on the head bar and grouped by shape in **Project Doctor > Reading**.
- **Files, and what a game may trust** - Godot's two places said where a path is typed: every path field carries a muted lead naming `user://` (the player's folder) or `res://` (read-only once exported), and Doctor > Files knows the export trap, the folder that exists on one computer, and the read of a file that is not there yet. A guard is written INTO the emitted line rather than performed behind it - **Read Text File (or a fallback)** takes the answer you would rather have in a second slot, and the make-the-folder prelude sits above the write where you can read it. A `.csv` reads through the engine's own `FileAccess.get_csv_line` as well as through the sheet's own parser, and writes back through `store_csv_line`. Content the player brings comes in through two doors - **On Files Dropped** and the two **Ask** rows, whose answers are events because a chooser answers minutes later - and arrives as a texture, a sound or a table, never as code. **Pack / Unpack Zip** emit their loops where they can be read, and the unpack's guard compares every entry's resolved path against the folder before a byte is written. The new **Folder Watcher** pack calls itself a poll, because Godot raises no file-change notification at run time. A file-handling project you already wrote opens on those rows byte for byte.
- **An object's own state, which is a variable** - patrolling, chasing, staggered is the pattern every event sheet already builds and every Godot project writes as an enum, so that is exactly what it is here: **Declare states…** on the sheet head writes five ordinary declarations (`enum State`, the `state` variable whose own setter announces the change, `previous_state`, `state_entered_msec`, `signal state_changed`) and four rows follow - **Is in `<state>`**, **Is in `<state>` for over `<seconds>`s**, **Was in `<state>`**, **Go to `<state>`** - with **On entering** / **On leaving** for the moment itself, leaving first. No new row kind, no group that only runs in a state, and no graph view: the band on the head is the diagram. Every state is offered from that object's own declarations as you type, so a state is picked rather than remembered; the Doctor knows the state nothing reaches and the state a row names that this object never declared; and while the game runs the band carries `current: Chase · 3.2 s`, a timed row shows `3.2 of 6` beside the question it asks, and the Debugger's **Trail** tab says what the machine just did as past-tense sentences. The `enum` + `var state` + `match state:` machine every Godot tutorial writes by hand opens as those rows and saves back byte for byte. ([guide](docs/GUIDE-STATES.md))
- **Scenes, in the engine's own words** - a scene is two things wearing one word, so the vocabulary keeps them apart: a place the player travels to is a **Layout**, a file holding a branch of nodes is a **Scene**. **Add Layout On Top** puts a pause menu, an inventory or a dialogue box OVER the running game, parented to the tree root so it outlives a change of layout beneath it, with **Remove Layout On Top** and **Layout Is On Top** as its pair and the menu's own Process Mode as the one thing to know. Godot's `%name` mark is a word the sheet speaks both ways - the picker's **%names** section scopes to the class the `.tscn` says a marked node is, a line already written on a `%Name` reads as a row on that object, and a field holding a path offers to make the mark through the editor's own undo. Spawning says the same sentence in three dimensions, with four words for where (**Place Of (3D)**, **Random Place Inside Box**, **Random Place Inside Sphere**, **Random Place Around (3D)**) and no dishonest one-liner for off-screen. **On Node Joins Group** / **On Node Leaves Group** are the scene tree's own announcements, with the shipped **Is In Group** condition put under the trigger as a row you can see and delete - the future tense of Connect Group Signal's present. **Save Branch As Scene File** writes out what a player built, doing the owner walk first (a node made while the game runs is owned by nothing, so a plain pack saves a scene holding one node and reports success), and **Scene File Is Data-Only** reads a `.tscn`'s resource table as text before anything builds it, because a scene file can name a script. An editor tool's whole event is one Ctrl+Z: **Set Property / Add Node / Remove Node (Undoable)** compile to the engine's own undo manager, and the compiler brackets each event once. A tool or a scene script you already wrote opens on those rows byte for byte. ([guide](docs/GUIDE-SCENES.md))
- **Autocomplete everywhere** - one seam (`EventSheets.completions_for`), one popup and one set of keys behind every field that holds a name: the `ƒx` boxes, the value you double-click inside a row, name, type and file fields. **Quick add** reads a whole typed sentence - `boss fla 0.4` ranks **Boss ▸ Flash white for 0.4 s** first with the 0.4 already in it. A keystroke never scans the project: each kind's list is built once and only filtered afterwards.

Opening the FPS Controller pack as a sheet went from 6.8 s to 0.9 s and the editor boot subtree from 583 ms to 193 ms. On a fabricated project ten times this one - 1,000 scripts, 300 scenes, 100 shaders, every pack installed - enabling the plugin costs **270 ms**, the first sheet tab of a session **2,310 ms**, one keystroke in a completing field **2.2 ms**, and a 4,000-line script opens in **7.4 s** where it used to take 11.5 s; eleven such budgets are now tests.

**The editor reads itself** - the plugin's own source opens as event sheets in its own repository: **886 files, 884 of them at zero script blocks (9 block lines in 189,540 rows), and 89% of those rows in the sheet's own words**. A rotating sample is round-tripped byte-exact and held under a measured per-group ceiling on every run (`tests/plugin_reads_itself_test.gd`), and Tools ▸ Project Doctor reports the same numbers per group.

**Quality** - 20,412 assertions across 717 test files, all green, CI-gated on every push; byte-exact golden round-trips guard the lossless rules; the save backend is pinned across all 18 seams and six formats; showcases are live-verified on camera. **Verified on Godot 4.7 stable.** Generated code never depends on the plugin, templates bake at apply time, and output is performance-identical to hand-written GDScript - all test-enforced.
