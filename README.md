...
And "opens as a sheet" means it *reads* as one. Measured over 206 hand-written files in this repo - ordinary style-guide GDScript nobody wrote for a demo - **99.95% of code lines arrive as structure** (functions, events, actions, notes, declarations) rather than a wall of code, with a byte-exact round-trip on every one. What legitimately stays code is code with no structured equivalent, and it stays honest and editable, never mangled. There is a suite gate on that number, because a fixture written to suit the lifter cannot notice that real code does not look like it.
