...
static func run() -> bool:
	...
	# Batch 14's pins parcel: +14 on the Pin pack (rope, bar, soft, spring, the two axis rows, size,
	# point, path, path progress, the axes knob, Is Taut and two expressions) and +24 for the whole
	# Pin 3D twin - the same rows plus a Z axis row and a Z offset expression: 38 in all.
	all_passed = _check("fleet-wide declared verbs count", total_verbs, 1283 + 3 + 19 + 2 + 38) and all_passed
