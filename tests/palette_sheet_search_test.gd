...
static func run() -> bool:
	var ok: bool = true
	var PATHS: PackedStringArray = _paths()

	var matches: Array = EventSheetCommandPalette.filter_sheets(PATHS, "player")
	...
	var all: Array = EventSheetCommandPalette.filter_sheets(PATHS, "")
	...
	var sub: Array = EventSheetCommandPalette.filter_sheets(PATHS, "enmy")
	ok = _check("subsequence 'enmy' finds enemy.tres", _has(sub, "enemy.tres"), true) and ok

	ok = _check("a no-match query returns nothing", EventSheetCommandPalette.filter_sheets(PATHS, "zzz").size(), 0) and ok
