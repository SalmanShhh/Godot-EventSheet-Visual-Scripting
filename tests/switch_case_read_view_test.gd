...
static func run() -> bool:
	...
	if case_rows.size() == 2:
		...
		# M37 - Construct has no switch, so a match on a plain value states its branches as the
		# if / else-if / else chain a Construct user reads: the first case says the TEST (subject and
