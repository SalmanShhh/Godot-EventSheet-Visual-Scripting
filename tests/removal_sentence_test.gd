# Godot EventSheets - destroying a thing, without leaving a ghost behind.
#
# What this gate holds, in the order the mistakes actually happen:
#   1. THE LINES. Each destroy row is a plain queue_free with a stated wait in front of it, and all
...
static func _test_the_descriptors_say_what_they_emit() -> bool:
	...
	passed = _check("Destroy Now is the plain call",
		str(by_id.get("DestroyNow", ACEDescriptor.new()).codegen_template), "{object}.queue_free()") and passed
	passed = _check("Destroy After Seconds hangs the free off a scene-tree timer",
		str(by_id.get("DestroyAfterSeconds", ACEDescriptor.new()).codegen_template),
		"get_tree().create_timer({seconds}).timeout.connect({object}.queue_free)") and passed
	passed = _check("Fade Out Then Destroy tweens, waits, and asks again before it destroys",
		str(by_id.get("FadeOutAndDestroy", ACEDescriptor.new()).codegen_template),
	...
	passed = _check("a destroy row opens on this node", _default_of(by_id.get("DestroyNow", null), "object"), "self") and passed
	passed = _check("the timer row opens on two seconds", _default_of(by_id.get("DestroyAfterSeconds", null), "seconds"), "2.0") and passed
	passed = _check("the fade row opens on half a second", _default_of(by_id.get("FadeOutAndDestroy", null), "seconds"), "0.5") and passed
	# The thing people get wrong about queue_free is WHEN, so the row has to say it.
	passed = _check("Destroy Now teaches that the deletion lands at the end of the frame",
		str(by_id.get("DestroyNow", ACEDescriptor.new()).description).contains("END of this frame"), true) and passed
	# On Destroyed is the shipped trigger and is pointed AT rather than duplicated here.
	passed = _check("the question points at the shipped trigger for a node's own destruction",
	...
	passed = _check("Destroy Now leaves the reading of a queue_free line to the shipped row",
		EventSheetACELifter.REVERSE_LIFT_EXCLUDED_ACE_IDS.has("DestroyNow"), true) and passed
...
static func _test_the_rows_write_their_lines() -> bool:
	var passed: bool = true
	passed = _check("Destroy Now writes the call on the object it names",
		_compiled_with("DestroyNow", {"object": "$Enemy"}, "user://eventforge_remove_now.gd")
			.contains("\t$Enemy.queue_free()"), true) and passed
	passed = _check("Destroy After Seconds writes the timer, the signal and the free",
		_compiled_with("DestroyAfterSeconds", {"object": "$Enemy", "seconds": "2.0"}, "user://eventforge_remove_later.gd")
			.contains("\tget_tree().create_timer(2.0).timeout.connect($Enemy.queue_free)"), true) and passed
	var faded: String = _compiled_with("FadeOutAndDestroy", {"object": "$Ghost", "seconds": "0.5"},
		"user://eventforge_remove_faded.gd")
	passed = _check("Fade Out Then Destroy waits for its own tween",
...
static func _test_the_guard_is_written_where_it_is_needed() -> bool:
	...
	stored.events.append(_event([_action("DestroyNow", {"object": "held_enemy"})]))
	var stored_output: String = _compile(stored, "user://eventforge_remove_stored.gd")
	passed = _check("a stored node reference is destroyed inside the guard",
	...
	# that `if`), so a destroy row in a different event does not compile whether it is wrapped or not:
	...
	across.events.append(_event([_action("DestroyNow", {"object": "boss"})]))
	...
	passed = _check("the destroy row there is the plain call it always was",
	...
	same.events.append(_event([
		...
		_action("DestroyNow", {"object": "boss"})
	]))
	var same_output: String = _compile(same, "user://eventforge_remove_same_event.gd")
	passed = _check("a copy destroyed in the event that made it is not guarded",
	...
	for object_text: String in ["self", "$Enemy", "%Boss"]:
		passed = _check("destroying %s needs no guard" % object_text,
			_compiled_with("DestroyNow", {"object": object_text}, "user://eventforge_remove_plain.gd")
	...
	waited.events.append(_event([_action("DestroyAfterSeconds", {"object": "held_enemy", "seconds": "2.0"})]))
...
## A destroy inside a match case is the same line as a destroy beside it, on the same name, in the
## same file - and a state machine puts its destroys in the case. The rule has to reach both emission
...
static func _test_the_guard_reaches_inside_a_match_case() -> bool:
	...
	branch.pattern = "0"
	branch.events.append(_action("DestroyNow", {"object": "held"}))
	...
	event.actions.append(_action("DestroyNow", {"object": "held"}))
	sheet.events.append(event)
	var output: String = _compile(sheet, "user://eventforge_remove_match.gd")
	var passed: bool = true
	passed = _check("the destroy inside the case is guarded",
...
static func _test_the_guard_stands_down_when_the_sheet_already_asked() -> bool:
	...
	var event: EventRow = _event([_action("DestroyNow", {"object": "held_enemy"})])
	...
	inner.actions.append(_action("DestroyNow", {"object": "held_enemy"}))
	...
	child.actions.append(_action("DestroyNow", {"object": "held_enemy"}))
	...
	var negated_event: EventRow = _event([_action("DestroyNow", {"object": "held_enemy"})])
...
static func _test_the_echo_is_the_line_the_file_holds() -> bool:
	...
	var plain: EventRow = _event([_action("DestroyNow", {"object": "self"})])
...
static func _test_the_hand_written_chains_lift() -> bool:
	...
	for spelling: String in [
		...
		passed = _check("\"%s\" opens as a destroy row" % spelling, matched.is_empty(), false) and passed
		if matched.is_empty():
			continue
...
## The Destroy module's descriptors by id. Loaded BY PATH so the test does not wait on the editor
