...
# sheets to "". A save must never happen as a side effect - the diff compiles to a temporary path only.
@tool
...
static func run() -> bool:
	...
	# Retune one line in a block that STAYS raw (_get_pool anchors as a real EventFunction now,
	# so the probe edits the host-binding _enter_tree - an engine virtual the lift always keeps
	# raw) and diff again, WITHOUT saving. The opened-pack diff attributes the change to EXACTLY
	# the edited row.
	var edited: RawCodeRow = null
	for row: Variant in sheet.events:
		if row is RawCodeRow and (row as RawCodeRow).code.contains("func _enter_tree()"):
			edited = row as RawCodeRow
	ok = _check("a raw block remains to edit (engine virtuals stay raw)", edited != null, true) and ok
	if edited == null:
		dock.free()
		return ok
	edited.code = edited.code.replace("if host == null:", "if host == null and is_inside_tree():")
	...
	ok = _check("the replaced disk line is listed as removed",
		Array(summary.get("removed_lines", PackedStringArray())).any(
			func(line: Variant) -> bool: return str(line) == "if host == null:"), true) and ok
...
static func _check(label: String, actual: Variant, expected: Variant) -> bool:
	return SUPPORT.check("sheet_diff_test", label, actual, expected)
