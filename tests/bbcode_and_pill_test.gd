...
#
# A condition/action cell whose display text carries BBCode-lite ([b]/[i]/[color]) draws styled, with the
...
# Variable rows no longer show a scope pill ("local"/"global") - it confused users.
@tool
...
static func run() -> bool:
	...
	# When the AUTHOR's display template carried markup (the one-shot flag is set), the cell renders
	...
	# FIX (review): a USER param value/note containing literal BBCode, in a PLAIN-template cell (flag
	...
	# Hover tooltip: a custom rich widget only when the description has markup.
	...
	# No variable row shows a scope pill anymore.
