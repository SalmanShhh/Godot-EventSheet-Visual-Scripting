...
static func run() -> bool:
	...
	all_passed = _check("grouping leaves a short toolbar",
		toolbar_editor._toolbar.get_child_count() <= 18, true) and all_passed  # +1: the Simple Mode pill, +1: the Settings menu (Words)
	...
	all_passed = _check("Sheet/Add/Edit/View menus carry the consolidated actions",
		sheet_menu != null and sheet_menu.get_popup().item_count == 21  # +New shared sheet… (V11), +Save as Text…, +New Behaviour Addon…, +Teach a Verb, +Inspector Designer, +New Editor Tool…, +New Custom Resource…, +Publish New Version…, +separator +Name Raw Calls…
		and add_menu != null and add_menu.get_popup().item_count == 14 + 1 + EventSheetBlockRegistry.addable_kinds().size()  # +separator +the three event-shape commands +separator +Code action, then +separator + one item per registered Custom Block kind; +1: Global Variable… beside Instance Variable…; +2: separator + Pattern…; +1: Include sheet… (V11)
		and edit_menu != null and edit_menu.get_popup().item_count == 10
		and view_menu != null and view_menu.get_popup().item_count == 38, true) and all_passed  # +1: Auto-apply while debugging (V8), +1: Patterns, +1: Reset Zoom, +1: Properties Bar, +1: Open Sheets Panel, +1: Language submenu, +1: Preview In Language submenu (the GAME's locales), +1: Object Icons toggle, +1: Event Numbers toggle, +1: Outline, +1: Aligned Object Columns, +1: Compact Rows, +1: Row Hit Counts, +1: Humanized Names, +1: Familiar Words, +6: the collapse sweeps (separator + Collapse All + Expand All + Expand To Level 1/2/3)
