...
static func run() -> bool:
	...
	all_passed = _check("grouping leaves a short toolbar",
		toolbar_editor._toolbar.get_child_count() <= 22, true) and all_passed  # -1: the Simple Mode pill left the strip, -1: the Settings menu retired into Tools, -4: Sheet/Edit/View/Tools became submenus of one Menu, -1: Add became a submenu of it too, +1: that Menu, +2: the Undo and Redo icons, +1: the play button's slot, +1: the chevron
