...
static func run() -> bool:
	...
	all_passed = _check("grouping leaves a short toolbar",
		toolbar_editor._toolbar.get_child_count() <= 21, true) and all_passed  # -1: the Simple Mode pill left the strip, -1: the Settings menu retired into Tools, -4: Sheet/Edit/View/Tools became submenus of one Menu, -1: Add became a submenu of it too, -1: the theme OptionButton became View ▸ Sheet theme, +1: that Menu, +2: the Undo and Redo icons, +1: the play button's slot, +1: the chevron
	...
	# below still pin. Add joined them: the strip stopped fronting the adding, because the canvas's
	# own corner links, the keys, the Ghost Row and the right-click menus already do.
	...
	var add_menu: PopupMenu = menu_popup.find_child("EventSheetAddMenu", true, false) as PopupMenu
	...
	all_passed = _check("Sheet/Add/Edit/View menus carry the consolidated actions",
		...
		and add_menu != null and add_menu.item_count == 21 + 1 + EventSheetBlockRegistry.addable_kinds().size()  # +separator +the three event-shape commands +separator +Code action, then +separator + one item per registered Custom Block kind; +1: Global Variable… beside Instance Variable…; +2: separator + Pattern…; +1: the Declare submenu; +6: the five keyed reflexes (Event/Condition/Action/Group/Comment) and their separator, which is what the strip's four Add buttons stopped being the only door to
		and edit_menu != null and edit_menu.item_count == 10
		and view_menu != null and view_menu.item_count == 59, true) and all_passed  # +1: Reset Lane Split (the boundary drag moved to the column header, so the way back to the default is a menu item); +1: Sheet theme (the retired theme picker's home); +1: Full toolbar (the chevron's twin); +1: Costs In The Sheet; +2: Follow Scene Selection + Debugger…; +3: the Arrange by and Saved Views submenus + Show Events in the Scene; +5: Ask…, separator, Reduced Motion, Speak This Row, Object Properties; +3: Minimap, Sheet Map…, History… (batch-10 UX 2); +1: Auto-apply while debugging; +2: Project bar + Add toolbar; +1: Patterns, +1: Reset Zoom, +1: Properties Bar, +1: Open Sheets Panel, +1: Language submenu, +1: Preview In Language submenu (the GAME's locales), +1: Object Icons toggle, +1: Event Numbers toggle, +1: Outline, +1: Aligned Object Columns, +1: Compact Rows, +1: Row Hit Counts, +1: Humanized Names, +1: Familiar Words, +6: the collapse sweeps (separator + Collapse All + Expand All + Expand To Level 1/2/3); +1: Variable rows
