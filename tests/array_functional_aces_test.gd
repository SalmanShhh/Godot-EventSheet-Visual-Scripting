...
static func run() -> bool:
	...
	ok = _check("filter template is the plain Array.filter one-liner",
		str(by_id["ArrayFilter"].codegen_template), "{var_name}.filter(func(x): return {predicate})") and ok
	ok = _check("reduce template folds acc + x from a seed",
		str(by_id["ArrayReduce"].codegen_template), "{var_name}.reduce(func(acc, x): return {expression}, {seed})") and ok
	...
	var filter_line: String = ActionCodegen._apply_template(str(by_id["ArrayFilter"].codegen_template), {"var_name": "arr", "predicate": "x > 0"})
	var map_line: String = ActionCodegen._apply_template(str(by_id["ArrayMap"].codegen_template), {"var_name": "arr", "expression": "x * 2"})
	var reduce_line: String = ActionCodegen._apply_template(str(by_id["ArrayReduce"].codegen_template), {"var_name": "arr", "expression": "acc + x", "seed": "0"})
	var any_line: String = ActionCodegen._apply_template(str(by_id["ArrayAny"].codegen_template), {"var_name": "arr", "predicate": "x > 0"})
	var all_line: String = ActionCodegen._apply_template(str(by_id["ArrayAll"].codegen_template), {"var_name": "arr", "predicate": "x > 0"})
