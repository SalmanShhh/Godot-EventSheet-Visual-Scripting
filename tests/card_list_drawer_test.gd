...
static func _prefab_pins() -> bool:
	...
	ok = _eq("line still titles p1/p2/p3 as End X/End Y/Thickness", _field_labels(schema, "line"), "Shape|Offset X|Offset Y|Scale|End X|End Y|Thickness|Colour|End caps|Dashed|Dash|Gap|Offset|Style") and ok
	ok = _eq("circle still titles p1 as Radius", _field_labels(schema, "circle"), "Shape|Offset X|Offset Y|Radius|Colour") and ok
	ok = _eq("a stamp still names its texture", _field_labels(schema, "stamp"), "Shape|Offset X|Offset Y|Scale|Spin|Texture|Colour") and ok
