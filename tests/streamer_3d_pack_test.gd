...
static func _a_streamer_whose_node_is_gone_parks_its_tick(stub: GDScript) -> bool:
	...
	# chunks, one a frame, around a place the game had left. With Stream Height off - the default,
	# a world that is flat - that radius is the square of cells at the follower's own level.
	var wide: Node = stub.new()
	var followed: Node3D = Node3D.new()
	wide.cell_size = Vector3(100.0, 100.0, 100.0)
	...
	return SUPPORT.pins(TEST, [
		...
		["a radius that has a node to measure from wants the cells around it",
...
## "chunk_3_4_0.tscn", found nowhere, and written down as a HOLE - which is remembered for ever, so
...
static func _a_chunk_asked_for_before_a_folder_waits_for_one(stub: GDScript) -> bool:
	var streamer: Node = stub.new()
	streamer.cell_size = Vector3(100.0, 100.0, 100.0)
	streamer.keep_chunk(Vector3i(3, 4, 0))
	streamer._process(0.016)
	...
	return SUPPORT.pins(TEST, [
		...
		["and the moment a folder is named, that is the chunk it asks for", asked_for_now,
			["res://world/chunks/chunk_3_4_0.tscn"] as Array[String]],
	])
