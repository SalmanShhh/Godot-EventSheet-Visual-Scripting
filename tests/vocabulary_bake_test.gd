...
const TEMP_CATALOG: String = "user://temp_bake_catalog.tres"
const TEMP_SCRIPT: String = "user://temp_bake_provider.gd"

const SOURCE: String = """class_name TempBakeProvider
extends Node
...
static func run() -> bool:
	var ok: bool = true
	EventSheetVocabularyCatalog.reset_for_tests(TEMP_CATALOG)
	...
	EventSheetVocabularyCatalog.set_override("TempBakeProvider", "method:take_damage",
		{"display_name": "Wound", "category": "Combat"})
	EventSheetVocabularyCatalog.set_override("TempBakeProvider", "method:heal", {"hidden": true})
	EventSheetVocabularyCatalog.set_override("OtherClass", "method:zzz", {"display_name": "Nope"})
	var edits: Array = EventSheetVocabularyCatalog.bake_edits_for("TempBakeProvider")
	...
	file.close()
	var result: Dictionary = EventSheets.bake_overrides(TEMP_SCRIPT, "TempBakeProvider")
	...
	var baked: String = FileAccess.get_file_as_string(TEMP_SCRIPT)
	...
	for code_line: String in ["func take_damage(amount: float) -> void:", "func heal(amount: float) -> void:",
			"class_name TempBakeProvider", "extends Node"]:
	...
	ok = _check("baked overrides leave the catalog",
		EventSheetVocabularyCatalog.bake_edits_for("TempBakeProvider").size(), 0) and ok
	...
	var empty: Dictionary = EventSheets.bake_overrides(TEMP_SCRIPT, "TempBakeProvider")
	...
	var rewritten: FileAccess = FileAccess.open(TEMP_SCRIPT, FileAccess.WRITE)
	rewritten.store_string(SOURCE)
	rewritten.close()
	EventSheetVocabularyCatalog.set_override("TempBakeProvider", "method:take_damage", {"display_name": "Wound"})
	EventSheetVocabularyCatalog.set_override("TempBakeProvider", "method:gone_away", {"display_name": "Ghost"})
	var partial: Dictionary = EventSheets.bake_overrides(TEMP_SCRIPT, "TempBakeProvider")
	...
	var remaining: Array = EventSheetVocabularyCatalog.bake_edits_for("TempBakeProvider")
	...
	var stranger: String = "user://temp_bake_stranger.gd"
	var other: FileAccess = FileAccess.open(stranger, FileAccess.WRITE)
	other.store_string("class_name TempBakeStranger\nextends Node\n")
	other.close()
	var before_count: int = EventSheetVocabularyCatalog.bake_edits_for("TempBakeProvider").size()
	var wrong: Dictionary = EventSheets.bake_overrides(stranger, "TempBakeProvider")
	...
	ok = _check("and destroys NO overrides",
		EventSheetVocabularyCatalog.bake_edits_for("TempBakeProvider").size(), before_count) and ok
	...
	if ResourceLoader.exists(TEMP_CATALOG):
		DirAccess.remove_absolute(TEMP_CATALOG)
	EventSheetVocabularyCatalog.reset_for_tests(TEMP_CATALOG)
	EventSheetVocabularyCatalog.set_override("TempBakeProvider", "property:set:health", {"display_name": "Assign HP"})
	EventSheetVocabularyCatalog.set_override("TempBakeProvider", "property:get:health", {"display_name": "HP"})
	var property_edits: Array = EventSheetVocabularyCatalog.bake_edits_for("TempBakeProvider")
