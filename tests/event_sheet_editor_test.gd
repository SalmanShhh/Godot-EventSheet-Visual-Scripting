...
static func run() -> bool:
    ...
    all_passed = _check("group row carries an editable title span (no redundant Group badge)",
        str((group_row.spans[1].metadata as Dictionary).get("edit_kind", "")) == "group_name" and not bool((group_row.spans[1].metadata as Dictionary).get("badge", false)), true) and all_passed
