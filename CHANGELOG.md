...
- **A second picture of the game: the Views shelf, 5 rows.** How big a view renders, which world it
  ...
  compiler's, the Doctor's and the canvas's coroutine lists - a lifted row carries only its id - and
  the pair of lines it emits reads back as the one row rather than as the wait and the write. Two rows are
...
- **Nine rendering words for the two questions a player actually asks.** **Render 3D At** renders
  ...
  **Pixel Size**. **Renderer Is** answers which of Godot's three the game is running on, which is
  what a graphics menu asks before it offers the settings only Forward+ draws. The frozen
  per-technique rows are untouched and are still the right rows for a menu that offers the switches
  separately.
...
- **Guide:**
  ...
  reference gains the nine rendering rows, the addon index gains Camera Rail, the pack count is
  re-measured at **119**, and the words land in all nine translation CSVs (**211 new keys**, most of
  them the dropdown labels a reader chooses between).

### Fixed in the camera and viewport pass

- The scene transition and its ease, the car's keyboard-style direction and the slide direction (on the verb and on Can Slide) are dropdowns over words, and a dropdown key is inserted into the call verbatim - so a row that picked "wipe" emitted an identifier nothing declares and the game did not parse. The three packs now carry the quotes in their templates, the way the Camera Rail packs do, and a test pins each shipped template and what a picked row emits through the compiler's own emitters.
