...
static func trunk_x(row_rect: Rect2, level: int, indent_width: int = EventSheetPalette.INDENT_WIDTH) -> float:
	return row_rect.position.x + float(EventSheetPalette.GUTTER_WIDTH) + float(level * indent_width) + 2.0
...
## the tighter row.
static func elbow_reach(indent_width: int = EventSheetPalette.INDENT_WIDTH) -> float:
	return float(indent_width) * 0.55 * EventSheetPalette.row_density()
...
static func draw_guides(control: Control, row_rect: Rect2, depth: int, guide_color: Color,
		indent_width: int = EventSheetPalette.INDENT_WIDTH) -> void:
	if control == null or depth < 1:
		return
	var line_width: float = EventSheetPalette.scaled_f(1.0)
	for level: int in range(depth):
		var x: float = trunk_x(row_rect, level, indent_width)
		control.draw_line(
	...
	var elbow_x: float = trunk_x(row_rect, depth - 1, indent_width)
	var elbow_y: float = row_rect.position.y + row_rect.size.y * 0.5
	control.draw_line(
		Vector2(elbow_x, elbow_y),
		Vector2(elbow_x + elbow_reach(indent_width), elbow_y),
		guide_color,
		line_width,
		true
	)
