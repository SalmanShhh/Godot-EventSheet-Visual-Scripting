...
func build_read_only_head_rows(rows: Array[EventRowData], sheet: EventSheetResource) -> Array[EventRowData]:
	...
	if pending_row != null:
		leftovers.append(pending_row)
	# Nothing that reads as a pack head - leave the sheet exactly as it was built. W16 - a page of the
	# vocabulary declares neither a trigger nor a knob and still has an identity worth stating: what it
	# publishes is the whole reason the file exists.
	var publishes_vocabulary: bool = bool(EventSheetEditorSourceFacts.facts(sheet).get("vocabulary_module", false))
	if not identity_seen or (triggers.is_empty() and knobs.is_empty() and not publishes_vocabulary):
		return rows
