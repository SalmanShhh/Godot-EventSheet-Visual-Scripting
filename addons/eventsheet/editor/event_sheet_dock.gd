...
# ── Project-wide find / replace / usages (advanced C3/GDevelop workflow) ─────────────
...
func _open_project_find(initial_query: String = "") -> void:
    if _project_find_window == null:
        ...
        add_child(_project_find_window)
...
func _run_project_find() -> void:
    ...
    if _current_sheet != null and not _current_sheet_path.is_empty() and not sheet_paths.has(_current_sheet_path):
        sheet_paths.append(_current_sheet_path)
    for sheet_path: String in sheet_paths:
        var sheet: EventSheetResource = _current_sheet if sheet_path == _current_sheet_path else load(sheet_path) as EventSheetResource
        for match_entry: Dictionary in find_in_sheet(sheet, needle):
            ...
            total += 1
    _set_status("Find in Project: %d match(es) for \"%s\"." % [total, needle])
...
func _on_project_find_activated() -> void:
    ...
    if _find_edit != null:
        _ensure_find_bar()
        _find_edit.text = _project_find_edit.text
...
func _run_project_replace() -> void:
    ...
    if needle.is_empty():
        _set_status("Type something to find first.", true)
        return
    ...
    for sheet_path: String in list_project_sheets():
        ...
        var counter: Dictionary = {"count": 0}
        _replace_in_rows(sheet.events, needle, replacement, counter)
        for function_entry: Variant in sheet.functions:
            if function_entry is EventFunction:
                _replace_in_rows((function_entry as EventFunction).events if not (function_entry as EventFunction).events.is_empty() else (function_entry as EventFunction).rows, needle, replacement, counter)
    ...
    if _current_sheet != null:
        _ensure_find_bar()
        _find_edit.text = needle
        _replace_edit.text = replacement
        _replace_all_in_sheet()
    _run_project_find()
    _set_status("Replace in Project: %s" % (", ".join(touched) if not touched.is_empty() else "only the open sheet had matches."))
