...
const HINT_PARAGRAPHS: Dictionary = {
	...
	"state_reference": "One of THIS object's declared states. They are declared once, on the states band of this sheet's head, and picking from this list is what makes a misspelled state impossible - a state that is not here has not been declared yet.",
	...
	"animation_state": "A state the attached scene's animation tree really declares, offered as the StringName the travel takes. A state the tree has never heard of goes amber with the nearest one offered: a misspelled state walks nowhere and reports nothing. A name built while the game runs is still typeable.",
	"blend_space": "A blend space in the attached scene's animation tree, listed with how many dimensions it has. That number is what decides the value beside it: a one-dimensional space takes a plain number, a two-dimensional one takes a Vector2. The name goes straight into the tree's own parameter path, so it is written bare rather than quoted.",
	"blend_layer": "A Blend2 or Add2 node in the attached scene's animation tree - the ones that mix one thing over another by an amount rather than by a position. The name goes straight into the tree's own parameter path, so it is written bare rather than quoted.",
	...
	"physics_layer_name_2d": "One 2D physics layer, by the name this project gave it in Project Settings > Layer Names > 2D Physics. The row stores the layer's NUMBER, which is what the engine's own call takes; the name is how the row reads it back, so renaming the layer renames the sentence and never touches the emitted line. A layer the project never named shows as its number with a field to name it.",
	"physics_layer_name_3d": "One 3D physics layer, by the name this project gave it in Project Settings > Layer Names > 3D Physics. The row stores the layer's NUMBER, which is what the engine's own call takes; the name is how the row reads it back, so renaming the layer renames the sentence and never touches the emitted line. A layer the project never named shows as its number with a field to name it.",
