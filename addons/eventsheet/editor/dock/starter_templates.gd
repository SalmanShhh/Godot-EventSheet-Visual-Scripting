...
func _build_template_menu_items() -> void:
	...
	_template_menu.add_item("Loot Chest (pity rolls)", 26)
	_template_menu.add_item("Stealth Guard (seen, heard, hunted)", 27)
	_template_menu.add_item("Boss Fight (phases and i-frames)", 28)
	_template_menu.add_item("Mission Timer (a clock with a deadline)", 29)
...
static func build_starter(template_id: int) -> EventSheetResource:
	...
	match template_id:
		...
		# Batch 13 - the four game-shape starters, renumbered past the editor shapes at merge.
		26: return _build_loot_chest_starter()
