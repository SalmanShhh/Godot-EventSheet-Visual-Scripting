...
| Rows | `synced_properties(sheet)` - every property of the sheet's own object that a `MultiplayerSynchronizer` in its scene keeps in step, one entry per property: `name`, `property_path`, `mode` ("always" / "on change" / "at spawn"), `synchronizer`, `synchronizer_path`, `scene_path`, `node_path`, `interval`, `public_visibility`. Read-only and derived on every ask - nothing about replication is stored in the sheet, so a `.gd` still round-trips byte for byte | `Array[Dictionary]` | no |
| Rows | `spawners_of(sheet)` - every `MultiplayerSpawner` this sheet is about: the ones in its own scene and the ones elsewhere whose list can make its scene, told apart by `relation` ("in_scene" / "spawns_this"). Entries carry `name`, `node_path`, `scene_path`, `spawn_path`, `spawn_limit`, `spawn_function`, `scenes`; `spawn_function` is read from the scene's GDScript, because Godot never stores a Callable in a `.tscn` | `Array[Dictionary]` | no |
| Rows | `scene_lights(sheet)` - every light in the scene (or scenes) that run this sheet's script, in scene order. Entries carry `name`, `path`, `class`, `kind` (the plain word - point / directional / omni / spot), `shadows`, `masks` and `shadow_masks` (the raw cull-mask text the scene file holds, `""` meaning the engine default - which is layer 1 for the two 2D masks and EVERY layer for a 3D light's `light_cull_mask`, so read an absent one with the default for that light's dimension), `reference` (the spelling a row addresses it by - `$Torch`, `$Props/Lantern`, `self`), `scene_path` and `properties`. The list the picker's "Lights in this scene" shelf and the head's `lit by` bands are built from. Read-only and derived on every ask, like `synced_properties`: nothing about the scene is stored in the sheet, so a `.gd` still round-trips byte for byte, and the Array you are handed is your own to sort or filter | `Array[Dictionary]` | no |
| Rows | `sheet_messages(sheet)` - every function the sheet publishes as a MESSAGE (the ones carrying an `@rpc`, which is what makes a call travel), in declaration order: `name`, `params`, `annotation` verbatim, `words` (that annotation read back in the sheet's own words, "from anyone · also here · reliable") and `note`, the amber line an option Godot does not take earns. This is the list a Send row picks from, so a pack building its own send surface offers exactly what the shipped one offers | `Array[Dictionary]` | no |
| Codegen | `message_annotation(original: String, answers: Dictionary)` - the `@rpc(...)` line a set of answers writes, given whatever the file already had. Answers are keyed by the `EventSheetMessageFacts.FIELD_*` words plus `channel`; a set that still MEANS what `original` said hands `original` back verbatim, so a pack editing a message through this cannot rewrite a `.gd` it did not change | `String` | no |
| Rows | `sheet_visibility_filters(sheet)` - every function of the sheet a `MultiplayerSynchronizer` asks whether a player may see its node, `{name, synchronizer}` in the order the rows name them (`synchronizer` is `""` when the asking row acts on the sheet's own node). Derived like `sheet_messages`: being a filter is the row that hands the function over, not a flag on the function | `Array[Dictionary]` | no |
...
| Seams | `register_card_schema(name, provider)` / `unregister_card_schema(name)` / `card_schema(name)` - the vocabulary behind a card-list Inspector drawer (`eventsheet:cards:kind=<k>,schema=<n>,stripes=<k>` on an Array of Dictionaries). `provider()` returns `{kinds: [{kind, category, label, help, fields, live, defaults, badge, actions}], label_key, enabled_key, stripes}`; a field's `drawer` word is the same word an export marker uses. A callable rather than a Dictionary because a pack's kinds are known when the Inspector opens, not when the plugin boots. Unregistered reads as `{}`, and such a list still draws and still saves its own bytes | `void` / `void` / `Dictionary` | no |
| Seams | `card_schema_from_aces(aces, fallback_category := "")` - a card schema DERIVED from ACE descriptors: one kind per verb, its label and help from the words the picker reads out, its fields from the verb's own parameters. The whole registration is then one line: `register_card_schema("my_steps", EventSheets.card_schema_from_aces.bind(my_aces))` | `Dictionary` | no |
| Seams | `register_inspector_preview(script_path, renderer)` / `unregister_inspector_preview(script_path)` / `inspector_preview_renderer_for(script_path)` - the picture at the top of an object's Inspector for a script that cannot ship `inspector_preview_texture(size)` itself. `renderer(object, size) -> Texture2D`, and it outranks the method. The object asks for the card with a `# @inspector_preview` comment | `void` / `void` / `Callable` | no |
| Seams | `register_toggle_icon_provider(name, renderer)` / `unregister_toggle_icon_provider(name)` / `toggle_icon_provider_for(name)` - pictures for a toggle-buttons drawer whose marker names this provider (`...:icons=<name>`; a source holding `%s` is a path pattern instead and needs nothing registered). `renderer(option, size) -> Texture2D`, called once per option as the row builds, at edit time only; return null and the button keeps its word | `void` / `void` / `Callable` | no |
