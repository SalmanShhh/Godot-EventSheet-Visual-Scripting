...
| Rows | `scene_lights(sheet)` - every light in the scene (or scenes) that run this sheet's script, in scene order. Entries carry `name`, `path`, `class`, `kind` (the plain word - point / directional / omni / spot), `shadows`, `masks` and `shadow_masks` (the raw cull-mask text the scene file holds, `""` meaning the engine default), `reference` (the spelling a row addresses it by - `$Torch`, `$Props/Lantern`, `self`), `scene_path` and `properties`. The list the picker's "Lights in this scene" shelf and the head's `lit by` bands are built from. Read-only and derived on every ask, like `synced_properties`: nothing about the scene is stored in the sheet, so a `.gd` still round-trips byte for byte | `Array[Dictionary]` | no |
