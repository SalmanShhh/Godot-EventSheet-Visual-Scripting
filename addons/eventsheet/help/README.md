...
- [Composing Juice: The Beat A Hit Is Made Of](GUIDE-COMPOSING-JUICE.md) - a hit written down once and played by name: the three homes one beat has, the Moment block and its four timing words, where a beat happened and how far it reaches, playing it backwards or turning it around mid-play, the Feedback Player's card list and every card addressed by its label, channels as groups, springs and curves under any property, blink patterns, audio sweeps and mix snapshots, haptics as shapes you own, the sequencer's grid on the song's clock, a bar whose underlay trails the value down, and what all of it costs on a phone and in a browser tab.
...
- [Damage, Status and Targeting](GUIDE-DAMAGE-STATUS-AND-TARGETING.md) - the chain from who fired it to what it hit: one metadata key for who made this and the bounded walk from the bullet to the turret to the person, friendly fire as one condition, damage that knows its kind and the fixed order it is mitigated in, the kinds as a file your game owns, the report written before the trigger fires, kill credit and assists that only count a hit that landed, a status as a word and a clock whose ticks are ordinary typed damage, the one number movement multiplies by, the difficulty factor a row scales a hit by and the assist that says it is one, the held enemy with the four ways a lock ends, the aim help the accessibility radius already governs, and the two things only 3D has.
