...
- **A pack that ships data, not just vocabulary.** Your loot pack's ACEs read a drop table. A `my_pack.drop_table` block puts that table IN the sheet as an editable row instead of a raw GDScript dictionary.
...
| `row_style(entry) -> String` | both | Optional ROW LOOK: which of the sheet's row shapes your rows draw as. `"section"` is the default flat block row; `"region"` is a fold mark - a dashed `#` badge in your kind's own tint (the `style()` accent), no chapter-bar height, and the row's own line echoed at the right edge. For a kind that stands for STRUCTURE - something that opens a stretch of the file and closes it again - this is how it gets the shape the sheet already has instead of a flat line. Your rows read with YOUR words: `summary()` is the title, the last line `emit()` writes is the echo. Folding a PAIR of rows into one is the built-in Region kind's own grammar, over the two fences it stores. Display only - never affects emission or the round-trip. |
...
**Scenario:** a sheet only works when another pack is installed, and the failure a user sees today is a missing-method error. A dependency block says so out loud, and reads back on open.
