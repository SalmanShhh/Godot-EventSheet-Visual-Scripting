...
- `$SpringBehavior.default_damping` inserts the **Default Damping** entry straight into any expression
- `$SpringBehavior.default_precision` inserts the **Default Precision** entry straight into any expression

The `$SpringBehavior` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("SpringBehavior")` chains,
