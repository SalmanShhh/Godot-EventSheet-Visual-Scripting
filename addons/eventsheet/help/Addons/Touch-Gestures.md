...
**A shape library makes taught shapes permanent.** Without a library attached, taught shapes live only for the run. Attach a **Touch Shape Library** data asset to the **shape_library** knob and teaching writes into it, **Save Shapes To Library** writes it back to its `.tres` file, and **Load Shapes From Library** (called for you when the behaviour starts) reads it back. Because it is a plain resource file it gets version-controlled, ships with the project, and can be swapped per level.
...
- **Teaching writes to the library, but only saving makes it permanent.** **Teach Shape From Stroke** puts the shape into the attached library in memory; **Save Shapes To Library** is what writes the `.tres` to disk. Without the save, the shape is gone when the game closes. With no library attached at all, taught shapes always last only for the run.
