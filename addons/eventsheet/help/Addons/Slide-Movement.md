...
- `$SlideMove.grid_size` inserts the **Grid Size** entry straight into any expression
- `$SlideMove.slide_speed` inserts the **Slide Speed** entry straight into any expression

The `$SlideMove` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("SlideMove")` chains,
