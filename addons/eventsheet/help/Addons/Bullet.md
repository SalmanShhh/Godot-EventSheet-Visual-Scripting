...
- **Gravity pulls along Gravity Angle.** At the default 90 it pulls straight down, and **Set Gravity Angle** re-points it at runtime, so a shot can arc sideways or upward. Leave gravity at `0` for a flat top-down bullet; use a positive value for arrows and grenades, or a negative value to make a bubble float up.
