...
**The tint is mixed, not set.** Every active effect's tint is multiplied together and applied to the host's modulate, and the colour underneath is worked out from what is on the sprite at that moment with the tint divided back out - so a second writer of the same colour is not overwritten when the last status leaves. That matters because the pack this one is most often paired with is one: Health's Grant Invincibility asks you to flicker with the Flash pack, and a tint that had captured the colour mid-flicker used to write that flicker back for good. A host that is not a CanvasItem - a 3D enemy, a plain Node - is simply left alone and its statuses work exactly the same. While a player has asked for no flashing, the shift is held under the same ceiling the screen effects use.
