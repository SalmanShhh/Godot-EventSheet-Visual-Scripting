...
- `$DialogueKitBehavior.advance_action` inserts the **Advance Action** entry straight into any expression
- `$DialogueKitBehavior.chars_per_second` inserts the **Chars Per Second** entry straight into any expression

The `$DialogueKitBehavior` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("DialogueKitBehavior")` chains,
