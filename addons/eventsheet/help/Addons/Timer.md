...
- `$TimerBehavior.duration` inserts the **Duration** entry straight into any expression
- `$TimerBehavior.repeating` inserts the **Repeating** entry straight into any expression

The `$TimerBehavior` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("TimerBehavior")` chains,
