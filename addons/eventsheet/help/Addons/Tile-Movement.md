...
- `$TileMovementBehavior.ai_controlled` inserts the **Ai Controlled** entry straight into any expression
- `$TileMovementBehavior.default_controls` inserts the **Default Controls** entry straight into any expression

The `$TileMovementBehavior` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("TileMovementBehavior")` chains,
