...
| **Extract-to-Function** | Turn a selection of actions into a named, reusable function in place; calls then render as a first-class **ƒ** function row. |
...
| **Simple Mode** | A beginner-friendly audience setting (offered on first run, and at **Menu ▸ View ▸ Simple Mode**) that trims the options and vocabulary shown, so a newcomer isn't handed the full registry at once. It gates the picker and the right-click menus, turns on reading mode, shows the beginner Add toolbar and the Project bar, closes the Properties bar, and pins variable rows to the sentence. It does **not** touch the toolbar strip - the strip rests on its own. Remembered per project. See [The Toolbar](GUIDE-THE-TOOLBAR.md). |
...
| **Curating a provider** | Editing what one of your own scripts publishes, from the preview table in **Sheet ▸ Custom Actions…**: tick a member to publish or not, correct its kind, rename it, re-file its category, and shape its parameters. **Curate Script…** then writes those choices back into the file as `## @ace_*` comments - the file is backed up first, only `##` comment lines are added or removed (no signature and no body is touched), and re-applying the same edits changes nothing. |
| **Forwarding shim** | A deprecated stand-in function under a published member's OLD name, appended by **Keep Old Name…** after you rename the real one. Renaming a member changes its identity in the picker, and every sheet already holds the old call *text*, so only a real member of the old name can keep those sheets working. The shim just forwards, and is hidden from the picker so it can't be added to new work. |
