...
## The one reading of an annotation spec line, shared with the importer's lifter and the compiler that
## writes those lines back. Named by path rather than by class: this file is reached on paths a boot
## takes, and naming the importer from the provider scanner would tie the two together for a helper
...
##
## The scan itself is the shared one, because the importer's lifter asks the same question of the
## same line and an answer only one of them gives is a vocabulary published from a reading the sheet
## never opens on.
func _brackets_balance(text: String) -> bool:
	return SpecText.brackets_balance(text)


## Splits on a separator only outside a quoted string AND outside a bracketed value, so
## `desc: "Slow, steady"` keeps its comma and `default: {"verb": "shake", "amount": 0.4}` stays ONE
## segment rather than being cut at the comma inside the literal.
##
## THE SPLIT IS THE LIFTER'S OWN, not a second reading of the same grammar. A pack's annotations are
## parsed by both, and a line the two disagreed about publishes one vocabulary and opens as another -
## with every gate green, because the byte gate asks whether the COMPILER reproduces the file and the
## compiler writes back what the LIFTER read. That is not a hypothetical: this file once toggled on
## `"` alone, so `default_code: 'a, b'` published as `'a` while the sheet opened on `'a, b'`, and
## a `default_code: "\""` holding an escaped quote swallowed the help text beside it.
func _split_outside_quotes(text: String, separator: String) -> Array[String]:
	var segments: Array[String] = []
	for part: String in SpecText.split_outside_quotes(text, separator):
