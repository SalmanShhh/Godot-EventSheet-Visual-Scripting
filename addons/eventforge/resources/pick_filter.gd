...
## Event-sheet loop index: name it and the loop counts 0, 1, 2... in a local of that name,
...
## on nested loops give each its own loop index counter. "" (the default) emits nothing, so
