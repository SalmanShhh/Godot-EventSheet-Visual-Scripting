...
# assigned from one, anything a `for` walks out of one, and any path written under a folder the same
# file watches or unpacks into. It keeps one set of names per FILE rather than one per function, so a
# name that is tainted anywhere is tainted everywhere in that file - which finds the very common shape
# where one handler stores the path on the object and another loads it, at the price of a name reused
# for two unrelated things in two functions reading as tainted in both. It does NOT follow a path
# across files, into an array element it cannot see the name of, or through a call into a function
# whose body it then walks. A quiet file is therefore not a proof; a loud one is a line to look at.
#
...
static func _is_outside(expression: String, tainted: Dictionary, folders: PackedStringArray) -> bool:
	for name_text: Variant in tainted.keys():
		if _mentions(expression, str(name_text)):
			return true
	for folder: String in folders:
		if _mentions(expression, folder) or expression.contains(folder):
			return true
		var literal: String = EventForgeFilePlaces.literal_of(folder)
		if not literal.is_empty() and expression.contains("\"%s/" % literal.trim_suffix("/")):
			return true
	return false
