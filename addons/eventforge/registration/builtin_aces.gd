...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(_make_descriptor("Core", "CompareValue", "Compare Value", ACEDescriptor.ACEType.CONDITION, "{left} {op} {right}", "", [_make_param("left", "String", "0", "Left", "Left-hand value/expression."), _make_param("op", "String", "==", "Operator", "Comparison operator.", "", COMPARISON_OPERATORS), _make_param("right", "String", "0", "Right", "Right-hand value/expression.")], "General Conditions", "{left} {op} {right}"))
	descriptors.append(_make_descriptor("Core", "IsActionPressed", "Input Action Pressed", ACEDescriptor.ACEType.CONDITION, "Input.is_action_pressed({action_name})", "", [_make_param("action_name", "String", "\"ui_accept\"", "Action Name", "Input action name to test.")], "Signals / Scene / Input", "Input action {action_name} is pressed"))
