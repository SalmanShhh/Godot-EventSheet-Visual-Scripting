...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(F.make_descriptor("Core", "LoopBreak", "Break Loop", ACEDescriptor.ACEType.ACTION, "break", "", [], "Loops", "Stop loop")
		.described("Stops the current loop early and skips any remaining items."))
	descriptors.append(F.make_descriptor("Core", "LoopContinue", "Continue Loop", ACEDescriptor.ACEType.ACTION, "continue", "", [], "Loops", "Next")
	...
	descriptors.append(F.make_descriptor("Core", "LoopIndex", "Loop Index", ACEDescriptor.ACEType.EXPRESSION, "loop_index", "", [], "Loops", "loop index (0, 1, 2…)")
		.described("Counts 0, 1, 2… for the current loop pass. Name the loop's index \"loop_index\" (the Loop index field on For Each / Repeat / While) and read it here."))
	descriptors.append(F.make_descriptor("Core", "LoopIndexNamed", "Loop Index Of", ACEDescriptor.ACEType.EXPRESSION, "{name}", "", [F.make_param("name", "String", "loop_index", "Index name", "The loop-index name you gave that loop (its Loop index field).", "expression")], "Loops", "loop index named {name}")
		.described("Reads a NAMED loop's counter, for nested loops: give the outer loop a distinct index name and read it from inside the inner one."))
	return descriptors
