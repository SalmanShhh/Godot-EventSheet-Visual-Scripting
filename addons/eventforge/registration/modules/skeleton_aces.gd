...
##
## THE 3D ROW TAKES A WORLD POSE AND THE ENGINE WANTS A SKELETON ONE, which is the same trap the Bone
## Position row above corrects for, in the other direction: `set_bone_global_pose_override` is global
## to the SKELETON, so a world transform handed to it straight lands the bone wherever the skeleton's
## own origin happens to be. The row divides the skeleton's transform back out, which is what makes a
## pose read from Bone Position - or built from any other node in the scene - mean what it says. The
## 2D twin needs no such correction: its pose is relative to the parent bone and says so.
static func _overrides(descriptors: Array[ACEDescriptor]) -> void:
	descriptors.append(F.act("SetBonePoseOverride", "Set Bone Pose Override", "{target.}set_bone_global_pose_override({target.}find_bone({bone}), {target.}global_transform.affine_inverse() * {pose}, {amount}, true)", CAT, "hold {bone} at {pose}", "Holds one bone in a pose of your own over whatever the animation is playing - the spine that twists on a hit, the head that stays level on a slope. The amount is how much of your pose wins: 1 for all of it, 0 to give the bone back to the animation.", "Skeleton3D").param("bone", "\"Head\"", "Bone", "The bone to hold, by the name the skeleton gives it.").param_typed("Transform3D", "pose", "Transform3D.IDENTITY", "Pose", "The pose to hold it in, in the world's own space.", "expression").param("amount", "1.0", "Amount", "How much of your pose wins over the animation: 1 for all of it, 0 for none.", "expression").param("target", "", "On node", "Hold a bone of another Skeleton3D instead. Leave blank for this node.", "expression"))
