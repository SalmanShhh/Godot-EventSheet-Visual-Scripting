...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(F.make_descriptor("Core", "VanishRespawnIn", "Vanish, Respawn In", ACEDescriptor.ACEType.ACTION, "visible = false\nif \"monitoring\" in self:\n\tset_deferred(&\"monitoring\", false)\nawait get_tree().create_timer(maxf({seconds}, 0.0)).timeout\nvisible = true\nif \"monitoring\" in self:\n\tset_deferred(&\"monitoring\", true)\nif has_method(&\"reset\"):\n\tcall(&\"reset\")", "", [F.make_param("seconds", "String", "10.0", "Seconds", "How long the pickup stays gone.", "expression")], "Time", "vanish, respawn in {seconds}s", "CanvasItem")
		.described("Hides this pickup (and pauses its Area sensing when it has any), waits, then brings it back - calling its reset() if it defines one. The health-pack-respawns-in-10s pattern as one row; it awaits, so it suspends like Wait."))
	...
	descriptors.append(F.make_descriptor("Core", "IsWithinDistanceMetric", "Is Within Distance (choose metric)", ACEDescriptor.ACEType.CONDITION, "([global_position.distance_to({other}.global_position), absf(global_position.x - {other}.global_position.x), absf(global_position.y - {other}.global_position.y), absf(global_position.x - {other}.global_position.x) + absf(global_position.y - {other}.global_position.y), maxf(absf(global_position.x - {other}.global_position.x), absf(global_position.y - {other}.global_position.y))][{metric}]) <= maxf({distance}, 0.0)", "", [F.make_param("other", "String", "get_parent()", "Of", "The other node.", "expression"), F.make_param("distance", "String", "64.0", "Distance", "In pixels (or use Tiles(n)).", "expression"), F.make_param("metric", "String", "0", "Measured As", "How the distance is counted - the platformer leash wants Horizontal only, the roguelike wants Grid steps.", "", [{"key": "0", "label": "Straight line"}, {"key": "1", "label": "Horizontal only"}, {"key": "2", "label": "Vertical only"}, {"key": "3", "label": "Grid steps"}, {"key": "4", "label": "King moves"}])], "Movement", "is within {distance} of {other}, measured as {metric}", "Node2D")
		.described("Is Within Distance with a geometry dropdown: straight line, horizontal or vertical only, grid steps (Manhattan), or king moves (Chebyshev) - one condition that fits platformers, roguelikes and strategy games alike."))
	...
	descriptors.append(F.make_descriptor("Core", "RegisterAsService", "Register As Service", ACEDescriptor.ACEType.ACTION, "var __svc_{uid}: Dictionary = get_tree().get_meta(&\"__ef_services\", {})\n__svc_{uid}[{service_name}] = self\nget_tree().set_meta(&\"__ef_services\", __svc_{uid})", "", [F.make_param("service_name", "String", "\"player\"", "Named", "A role, not a class - \"player\", \"audio_director\", \"wallet\". Registering the same name again replaces it.", "expression")], "Nodes", "register this as service [b]{service_name}[/b]")
		.described("Publishes this node under a short name anything can ask for, so no other row needs a path to it. Run it under On Ready. Registering the same name again replaces it, and a freed node answers as nothing at all, so a scene reload leaves no dangling reference behind."))
	descriptors.append(F.make_descriptor("Core", "ServiceNamed", "Service Named", ACEDescriptor.ACEType.EXPRESSION, "(get_tree().get_meta(&\"__ef_services\", {}).get({service_name}, null) if is_instance_valid(get_tree().get_meta(&\"__ef_services\", {}).get({service_name}, null)) else null)", "", [F.make_param("service_name", "String", "\"player\"", "Named", "The name the node registered itself under.", "expression")], "Nodes", "service named [b]{service_name}[/b]")
		.described("The node published under a name, or nothing at all when nobody registered it or it has been freed. Drop it wherever a node is asked for - it never crashes and never needs a path."))
	descriptors.append(F.make_descriptor("Core", "HasService", "Has Service", ACEDescriptor.ACEType.CONDITION, "is_instance_valid(get_tree().get_meta(&\"__ef_services\", {}).get({service_name}, null))", "", [F.make_param("service_name", "String", "\"audio_director\"", "Named", "The service name to test for.", "expression")], "Nodes", "service [b]{service_name}[/b] exists")
		.described("True when a node is registered under this name and is still alive. Guard an optional system with it - a missing audio director then costs you a skipped row instead of a crash."))
	...
	descriptors.append(F.make_descriptor("Core", "ForEachNodeThatCan", "For Each Node That Can", ACEDescriptor.ACEType.CONDITION, "([get_tree().get_root()] + Array(get_tree().get_root().find_children(\"*\", \"\", true, false))).filter(func(__can_{uid}): return __can_{uid}.has_method({method_name}))", "", [F.make_param("method_name", "String", "\"save_state\"", "Answers To", "The method name a node must have to be picked up, e.g. save_state or on_game_paused.", "expression")], "Nodes: Picking", "for each node that can [b]{method_name}[/b]")
		.described("Runs this event's actions once per node ANYWHERE in the tree that answers to a method name - the save sweep, the pause sweep, the shutdown sweep, with no list to keep in step. It walks the whole tree, so run it on an event, not every frame. Read the current one as `node`.").looping("node"))
	...
	descriptors.append(F.make_descriptor("Core", "DoAfterFrame", "Do After This Frame", ACEDescriptor.ACEType.ACTION, "(func(): {do}).call_deferred()", "", [F.make_param("do", "String", "queue_free()", "Do", "The action to run once this frame's work is finished - the only safe moment to add, free or reparent a node.", "expression")], "Time", "after this frame: [b]{do}[/b]")
		.described("Runs one action once the frame's physics and signal storm has finished, which is the only safe moment to add, free or reparent nodes. The number one crash a beginner hits is doing that from inside a collision callback; this is the fix, and the rows below it still run immediately."))
	descriptors.append(F.make_descriptor("Core", "CallLater", "Call Later", ACEDescriptor.ACEType.ACTION, "get_tree().create_timer(maxf({seconds}, 0.0)).timeout.connect(func(): {do}, CONNECT_ONE_SHOT)", "", [F.make_param("seconds", "String", "0.15", "After", "Seconds to wait before the action runs.", "expression"), F.make_param("do", "String", "queue_free()", "Do", "The action to run when the delay is up.", "expression")], "Time", "call later ([b]{seconds}[/b]s): [b]{do}[/b]")
		.described("Runs one action after a delay, with no Timer node and WITHOUT suspending - the rows below it keep running. The connection is one-shot, so a delayed beat can neither leak nor fire twice. The readable form of the small follow-up: eject the shell, close the door, play the second thud."))
	descriptors.append(F.make_descriptor("Core", "SetPropertyDeferred", "Set Property (after this frame)", ACEDescriptor.ACEType.ACTION, "{target}.set_deferred({property}, {value})", "", [F.make_param("target", "String", "self", "On", "Node whose property to set.", "expression"), F.make_param("property", "String", "\"visible\"", "Property", "Name of the property to set.", ""), F.make_param("value", "String", "true", "To", "The value to set it to.", "expression")], "Time", "set [b]{property}[/b] of [i]{target}[/i] to [b]{value}[/b] after this frame")
		.described("Sets a property at the end of the frame instead of right now. The one Godot insists on for collision shapes and monitoring: flipping those mid-physics is the error every beginner meets, and deferring is the documented answer."))
	...
	descriptors.append(F.make_descriptor("Core", "OnceThisFrame", "Only Once This Frame", ACEDescriptor.ACEType.CONDITION, "__once_frame_{uid}({key_id})", "", [F.make_param("key_id", "String", "\"rebuild_bag\"", "Named", "The gate's name. Every row using the same name folds into one run per frame.", "expression")], "Run Context", "only once this frame ( [b]{key_id}[/b] )")
		.stateful("var __once_frames_{uid}: Dictionary = {}\n\nfunc __once_frame_{uid}(key: String) -> bool:\n\tvar stamp: String = str(Engine.get_process_frames()) + \":\" + str(Engine.get_physics_frames())\n\tif str(__once_frames_{uid}.get(key, \"\")) == stamp:\n\t\treturn false\n\t__once_frames_{uid}[key] = stamp\n\treturn true")
		.evaluated_last()
		.described("True the FIRST time it is reached in a frame under this name, and false for every other time that frame - counting a physics tick as its own frame, so a row under On Physics Process is not folded in with the drawn frame around it. Fifty inventory changes then rebuild the panel once instead of fifty times. Give two rows the same name and they share one run per frame."))
	return descriptors
