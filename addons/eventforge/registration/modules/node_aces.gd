...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(F.make_descriptor("Core", "RemoveChild", "Remove Child", ACEDescriptor.ACEType.ACTION, "remove_child({node})", "", [F.make_param("node", "String", "get_child(0)", "Node", "Child to detach (by index here - no hardcoded name; pick one or use a scene-unique node). Not freed.", "expression")], "Nodes", "remove child {node}")
