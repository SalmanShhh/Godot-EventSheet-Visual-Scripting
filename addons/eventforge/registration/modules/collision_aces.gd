...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(F.make_descriptor("Core", "OverlapsBody", "Overlaps Body", ACEDescriptor.ACEType.CONDITION, "overlaps_body({body})", "", [F.make_param("body", "String", "self", "Body", "Body node to test for overlap.", "expression")], "Collisions", "overlaps body {body}", "Area2D"))
	descriptors.append(F.make_descriptor("Core", "OverlapsArea", "Overlaps Area", ACEDescriptor.ACEType.CONDITION, "overlaps_area({area})", "", [F.make_param("area", "String", "self", "Area", "Area node to test for overlap.", "expression")], "Collisions", "overlaps area {area}", "Area2D"))
