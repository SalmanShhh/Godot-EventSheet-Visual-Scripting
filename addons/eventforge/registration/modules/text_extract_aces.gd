...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(F.make_descriptor("Core", "TextBefore", "Text Before", ACEDescriptor.ACEType.EXPRESSION,
		"{text}.get_slice({marker}, 0)", "",
		[F.make_param("text", "String", SAMPLE_LINE, "Text", "The line to read a piece out of.", "expression"),
		F.make_param("marker", "String", "\" [\"", "Before", "The marker to stop at (the first one wins).", "expression")],
		TEXT_CAT, "the part of {text} before {marker}")
		.described("The part of the text before the first marker: Text Before(\"Ada [angry]: hi\", \" [\") is \"Ada\". When the marker is not there you get the whole text back, so nothing is silently lost."))
	...
	descriptors.append(F.make_descriptor("Core", "TextAfter", "Text After", ACEDescriptor.ACEType.EXPRESSION,
		"str((Array({text}.split({marker}, true, 1)) + [\"\"])[1])", "",
		[F.make_param("text", "String", SAMPLE_LINE, "Text", "The line to read a piece out of.", "expression"),
		F.make_param("marker", "String", "\"]: \"", "After", "The marker to start after (the first one wins).", "expression")],
		TEXT_CAT, "the part of {text} after {marker}")
		.described("Everything after the first marker: Text After(\"Ada [angry]: hi\", \"]: \") is \"hi\". Empty when the marker is not there, because then there is no \"after\"."))

	descriptors.append(F.make_descriptor("Core", "TextBetween", "Text Between", ACEDescriptor.ACEType.EXPRESSION,
		"str((Array({text}.split({open}, true, 1)) + [\"\"])[1]).get_slice({close}, 0)", "",
		[F.make_param("text", "String", SAMPLE_LINE, "Text", "The line to read a piece out of.", "expression"),
		F.make_param("open", "String", "\"[\"", "After", "The opening marker.", "expression"),
		F.make_param("close", "String", "\"]\"", "Before", "The closing marker.", "expression")],
		TEXT_CAT, "the part of {text} between {open} and {close}")
		.described("The part between two markers: Text Between(\"Ada [angry]: hi\", \"[\", \"]\") is \"angry\". Empty when the opening marker is missing, and the rest of the text when the closing one is.").featured())
	...
	descriptors.append(F.make_descriptor("Core", "NumberInText", "Number In Text", ACEDescriptor.ACEType.EXPRESSION,
		"(RegEx.create_from_string(\"-?[0-9]+(\\\\.[0-9]+)?\").search_all({text}).map(func(__m): return __m.get_string()) + [\"0\"]).front().to_float()", "",
		[F.make_param("text", "String", "\"Chapter 3\"", "Text", "Text that has a number somewhere in it.", "expression")],
		TEXT_CAT, "the first number in {text}")
		.described("The first number found anywhere in the text, whole or decimal: \"Chapter 3\" gives 3, \"v1.25-beta\" gives 1.25. You get 0 when there is no number at all, and you never write a pattern."))
	...
	descriptors.append(F.make_descriptor("Core", "SplitKeepingQuotes", "Split Keeping Quotes", ACEDescriptor.ACEType.EXPRESSION,
		"Array({text}.split(\"\\\"\")).reduce(func(__acc, __part): return [not __acc[0], __acc[1] + ([__part] if __acc[0] and not __part.is_empty() else Array(__part.split({separator}, false)))], [false, []])[1]", "",
		[F.make_param("text", "String", "\"give \\\"iron sword\\\" 2\"", "Text", "The line to split - a console entry, a search box, a pasted cell.", "expression"),
		F.make_param("separator", "String", "\" \"", "Separator", "What separates the pieces OUTSIDE quotes.", "expression")],
		TEXT_CAT, "pieces of {text}, keeping quotes together")
		.described("Splits text on a separator but keeps anything inside \"double quotes\" together as one piece, and drops the quotes: give \"iron sword\" 2 is three pieces, not four. Empty pieces are skipped, so runs of separators never produce blanks.").featured())
	...
	descriptors.append(F.make_descriptor("Core", "ExplainJsonProblem", "Explain JSON Problem", ACEDescriptor.ACEType.EXPRESSION,
	...
	descriptors.append(F.make_descriptor("Core", "ExplainTableProblem", "Explain Table Problem", ACEDescriptor.ACEType.EXPRESSION,
		"(func(__rows: Array, __cols: Array) -> String: return str((range(__rows.size()).map(func(__i): return (([\"row %d is not a record\" % (__i + 1)] if typeof(__rows[__i]) != TYPE_DICTIONARY else __cols.filter(func(__c): return not str(__rows[__i].get(__c, \"\")).strip_edges().is_valid_float()).map(func(__c): return \"row %d, column \\\"%s\\\": \\\"%s\\\" is not a number\" % [__i + 1, __c, __rows[__i].get(__c, \"\")])) + [\"\"]).front()).filter(func(__p): return not str(__p).is_empty()) + [\"\"]).front())).call({records}, {columns})", "",
		[F.make_param("records", "String", "[]", "Rows", "A list of records - one per row, each field reachable by column name.", "expression"),
		F.make_param("columns", "String", "[\"price\"]", "Number columns", "The columns that must hold numbers.", "expression")],
		TABLE_CAT, "what is wrong in {records}")
		.described("The first cell of a table that should be a number and is not, said out loud: \"row 12, column \"price\": \"abc\" is not a number\". Rows are counted from 1 over the rows you hold - Table From File has already used up the header line, so row 12 is line 13 of the file. A row that is not a record at all is reported too. Empty when every listed column checks out."))
	...
	descriptors.append(F.make_descriptor("Core", "MissingFields", "Missing Fields", ACEDescriptor.ACEType.EXPRESSION,
		"(func(__record: Variant, __names: Array) -> String: return \", \".join(PackedStringArray(__names.map(func(__n): return [str(__n).strip_edges(), __record.get(str(__n).strip_edges()) if __record != null else null]).filter(func(__p): return __p[1] == null or ((__p[1] is String or __p[1] is Array or __p[1] is Dictionary) and __p[1].is_empty())).map(func(__p): return __p[0])))).call({record}, Array({fields}.split(\",\")))", "",
		[F.make_param("record", "String", "{}", "Record", "A record or a resource to check.", "expression"),
		F.make_param("fields", "String", "\"id, name\"", "Fields", "The fields it must have filled in, separated by commas.", "expression")],
		RECORD_CAT, "the fields missing from {record}")
		.described("The listed fields that are missing or left blank, comma-separated, and empty when the record is complete. Blank means nothing there: null, empty text, an empty list or an empty record - a 0 is a real value and is never reported."))
