...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(F.make_descriptor("Core", "OnEnterTree", "On Created", ACEDescriptor.ACEType.TRIGGER, "", "_enter_tree", [], "Run Context", "Run when created", "Node")
		.described("Runs the moment this object is added to the scene - before its children exist, which is what makes it earlier than On Ready."))
	descriptors.append(F.make_descriptor("Core", "OnExitTree", "On Destroyed", ACEDescriptor.ACEType.TRIGGER, "", "_exit_tree", [], "Run Context", "Run when destroyed", "Node")
	...
	descriptors.append(F.make_descriptor("Core", "OnInputEvent", "On Input On This Object", ACEDescriptor.ACEType.TRIGGER, "", "_input_event", [], "Input", "On input on this object", "CollisionObject2D")
		.described("Runs when input lands on this object's own collision shape - a click, a drag or a touch that hit it rather than the world behind it."))
	...
	descriptors.append(F.make_descriptor("Core", "MoveAndSlide", "Move And Slide", ACEDescriptor.ACEType.ACTION, "{host.}move_and_slide()", "", [], "General Actions", "Move (and slide along what it hits)", "CharacterBody2D")
	...
	descriptors.append(F.make_descriptor("Core", "LimitSpeed", "Limit Speed", ACEDescriptor.ACEType.ACTION, "{host.}velocity = {host.}velocity.limit_length({max_speed})", "", [F.make_param("max_speed", "String", "400.0", "Max speed", "Fastest the body may travel, in pixels per second.", "expression")], "Movement", "Limit speed to {max_speed}", "CharacterBody2D")
		.described("Caps how fast the body can travel in any direction, keeping diagonal movement no faster than straight movement."))
	descriptors.append(F.make_descriptor("Core", "IgnoreCollisionsWith", "Ignore Collisions With", ACEDescriptor.ACEType.ACTION, "{host.}add_collision_exception_with({other})", "", [F.make_param("other", "String", "self", "Other", "The body to pass through from now on.", "expression")], "Movement", "Ignore collisions with {other}", "CharacterBody2D")
		.described("Lets this body pass through one other body from now on - a moving platform it rides, or the object that just fired it."))
	descriptors.append(F.make_descriptor("Core", "RotateToward", "Rotate Toward", ACEDescriptor.ACEType.ACTION, "{host.}rotation = lerp_angle({host.}rotation, {angle}, {rate} * {delta_t})", "", [F.make_param("angle", "String", "0.0", "Angle", "Angle to turn toward, in radians.", "expression"), F.make_param("rate", "String", "5.0", "Rate", "How much of the remaining turn is taken each second.", "expression"), F.make_param("delta_t", "String", "delta", "Delta", "Frame time; defaults to `delta`.", "expression")], "Movement", "Rotate toward {angle} at {rate} (per second)", "Node2D")
		.described("Turns the object toward an angle a little each frame instead of snapping to it, taking the shorter way round."))
