...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	# R38 - the sheet's second variable question, said as a sentence. `if alive:` reads "alive is
	# true" and `if not muted:` reads "muted is false"; inverting one flips the WORD rather than
	# adding a mark, which is why the one entry offers both.
	descriptors.append(F.make_descriptor("Core", "BoolVarIsTrue", "Boolean is true / is false", ACEDescriptor.ACEType.CONDITION, "{var_name} == {value}", "", [F.make_param("var_name", "String", "enabled_flag", "Variable", "Boolean variable to test.", "variable_reference"), F.make_param("value", "String", "true", "Is", "true or false.", "", PackedStringArray(["true", "false"]))], "Variables", "{var_name} is {value}")
		.described("True when a boolean variable holds the value you pick - the plain \"alive is true\" / \"muted is false\" question."))
	...
	# R38 - setting a flag is its own verb, because "Set boolean" is what somebody looks for. Same
	# line as Set value; the entry exists so the search finds it and the row reads as a flag.
	descriptors.append(F.make_descriptor("Core", "SetBoolVar", "Set boolean", ACEDescriptor.ACEType.ACTION, "{var_name} = {value}", "", [F.make_param("var_name", "String", "enabled_flag", "Variable", "Boolean variable to set.", "variable_reference"), F.make_param("value", "String", "true", "To", "true or false.", "", PackedStringArray(["true", "false"]))], "Variables", "Set {var_name} to {value}")
		.described("Sets a true/false variable to true or false - the flag half of Set value."))
