...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	# Array
	descriptors.append(F.make_descriptor("Core", "ArrayAppend", "Push Back", ACEDescriptor.ACEType.ACTION, "{var_name}.append({value})", "", [F.make_param("var_name", "String", "list", "Array", "Array variable.", "variable_reference:Array"), F.make_param("value", "String", "0", "Value", "Value to append.", "expression")], "Variables: Array", "Push back {value} to {var_name}")
	...
	descriptors.append(F.make_descriptor("Core", "ArrayRemoveAt", "Delete At", ACEDescriptor.ACEType.ACTION, "{var_name}.remove_at({index})", "", [F.make_param("var_name", "String", "list", "Array", "Array variable.", "variable_reference:Array"), F.make_param("index", "String", "0", "Index", "Index to remove.", "expression")], "Variables: Array", "Delete at {index} in {var_name}")
		.described("Removes the item at a specific position in an array."))
	descriptors.append(F.make_descriptor("Core", "ArrayErase", "Delete Value", ACEDescriptor.ACEType.ACTION, "{var_name}.erase({value})", "", [F.make_param("var_name", "String", "list", "Array", "Array variable.", "variable_reference:Array"), F.make_param("value", "String", "0", "Value", "First matching value to remove.", "expression")], "Variables: Array", "Delete value {value} from {var_name}")
	...
	descriptors.append(F.make_descriptor("Core", "WaitUntil", "Wait Until", ACEDescriptor.ACEType.ACTION, "var __wait_end_{uid}: int = Time.get_ticks_msec() + int(maxf({seconds}, 0.0) * 1000.0)\nvar __wait_ok_{uid}: bool = true\nwhile not ({check}):\n\tif {seconds} > 0.0 and Time.get_ticks_msec() >= __wait_end_{uid}:\n\t\t__wait_ok_{uid} = false\n\t\tbreak\n\tawait get_tree().process_frame\nset_meta(&\"__ef_wait_\" + str({wait_name}).to_utf8_buffer().hex_encode(), 1 if __wait_ok_{uid} else 2)\nif not __wait_ok_{uid} and has_signal(&\"verb_failed\"):\n\temit_signal(&\"verb_failed\", str({wait_name}), \"timed out after \" + str({seconds}) + \"s\")", "", [F.make_param("wait_name", "String", "\"loading\"", "Named", "A name for this wait, so Wait Succeeded / Wait Timed Out can say which one they mean.", "expression"), F.make_param("check", "String", "true", "Until", "The check to wait for, tested once a frame.", "expression"), F.make_param("seconds", "String", "5.0", "Give Up After", "Seconds before giving up. 0 waits forever.", "expression")], "Time", "wait [b]{wait_name}[/b] until [b]{check}[/b], give up after [b]{seconds}[/b]s")
		.described("Pauses this event until a check comes true, or until the give-up time passes. Read which happened on the next row with Wait Succeeded / Wait Timed Out, naming this wait. It polls once a frame, so prefer Wait For Signal when a real signal exists for what you are waiting on."))
	descriptors.append(F.make_descriptor("Core", "WaitForAllOf", "Wait For All Of", ACEDescriptor.ACEType.ACTION, "var __join_left_{uid}: Array[int] = [0]\nvar __join_conns_{uid}: Array = []\nfor __sig_{uid}: Signal in {signals}:\n\tvar __arity_{uid}: int = 0\n\tfor __info_{uid}: Dictionary in __sig_{uid}.get_object().get_signal_list():\n\t\tif str(__info_{uid}.get(\"name\", \"\")) == str(__sig_{uid}.get_name()):\n\t\t\t__arity_{uid} = (__info_{uid}.get(\"args\", []) as Array).size()\n\t__join_left_{uid}[0] += 1\n\tvar __cb_{uid}: Callable = func() -> void: __join_left_{uid}[0] -= 1\n\tif __arity_{uid} > 0:\n\t\t__cb_{uid} = __cb_{uid}.unbind(__arity_{uid})\n\t__sig_{uid}.connect(__cb_{uid}, CONNECT_ONE_SHOT)\n\t__join_conns_{uid}.append([__sig_{uid}, __cb_{uid}])\nvar __join_end_{uid}: int = Time.get_ticks_msec() + int(maxf({seconds}, 0.0) * 1000.0)\nvar __join_ok_{uid}: bool = true\nwhile __join_left_{uid}[0] > 0:\n\tif {seconds} > 0.0 and Time.get_ticks_msec() >= __join_end_{uid}:\n\t\t__join_ok_{uid} = false\n\t\tbreak\n\tawait get_tree().process_frame\nfor __conn_{uid}: Array in __join_conns_{uid}:\n\tif is_instance_valid((__conn_{uid}[0] as Signal).get_object()) and (__conn_{uid}[0] as Signal).is_connected(__conn_{uid}[1] as Callable):\n\t\t(__conn_{uid}[0] as Signal).disconnect(__conn_{uid}[1] as Callable)\nset_meta(&\"__ef_wait_\" + str({wait_name}).to_utf8_buffer().hex_encode(), 1 if __join_ok_{uid} else 2)\nif not __join_ok_{uid} and has_signal(&\"verb_failed\"):\n\temit_signal(&\"verb_failed\", str({wait_name}), \"timed out after \" + str({seconds}) + \"s\")", "", [F.make_param("wait_name", "String", "\"gate\"", "Named", "A name for this wait, so Wait Succeeded / Wait Timed Out can say which one they mean.", "expression"), F.make_param("signals", "String", "[]", "Signals", "The signals to join, as a list: [$Door.opened, $Camera.arrived].", "expression"), F.make_param("seconds", "String", "8.0", "Give Up After", "Seconds before giving up. 0 waits forever.", "expression")], "Time", "wait [b]{wait_name}[/b] for ALL of [b]{signals}[/b], give up after [b]{seconds}[/b]s")
		.described("Pauses this event until every signal in the list has fired at least once, or until the give-up time passes. It connects them ALL up front, one-shot, so nothing can fire into the gap between two waits - the bug a hand-written chain of awaits always has. An empty list succeeds at once."))
	descriptors.append(F.make_descriptor("Core", "WaitForAnyOf", "Wait For Any Of", ACEDescriptor.ACEType.ACTION, "var __any_won_{uid}: Array[String] = [\"\"]\nvar __any_conns_{uid}: Array = []\nfor __sig_{uid}: Signal in {signals}:\n\tvar __arity_{uid}: int = 0\n\tfor __info_{uid}: Dictionary in __sig_{uid}.get_object().get_signal_list():\n\t\tif str(__info_{uid}.get(\"name\", \"\")) == str(__sig_{uid}.get_name()):\n\t\t\t__arity_{uid} = (__info_{uid}.get(\"args\", []) as Array).size()\n\tvar __owner_{uid}: Object = __sig_{uid}.get_object()\n\tvar __won_{uid}: String = ((__owner_{uid} as Node).name + \".\" if __owner_{uid} is Node else \"\") + str(__sig_{uid}.get_name())\n\tvar __cb_{uid}: Callable = func() -> void: __any_won_{uid}[0] = __won_{uid} if __any_won_{uid}[0] == \"\" else __any_won_{uid}[0]\n\tif __arity_{uid} > 0:\n\t\t__cb_{uid} = __cb_{uid}.unbind(__arity_{uid})\n\t__sig_{uid}.connect(__cb_{uid}, CONNECT_ONE_SHOT)\n\t__any_conns_{uid}.append([__sig_{uid}, __cb_{uid}])\nvar __any_end_{uid}: int = Time.get_ticks_msec() + int(maxf({seconds}, 0.0) * 1000.0)\nwhile __any_won_{uid}[0] == \"\":\n\tif {seconds} > 0.0 and Time.get_ticks_msec() >= __any_end_{uid}:\n\t\tbreak\n\tawait get_tree().process_frame\nfor __conn_{uid}: Array in __any_conns_{uid}:\n\tif is_instance_valid((__conn_{uid}[0] as Signal).get_object()) and (__conn_{uid}[0] as Signal).is_connected(__conn_{uid}[1] as Callable):\n\t\t(__conn_{uid}[0] as Signal).disconnect(__conn_{uid}[1] as Callable)\nset_meta(&\"__ef_first_\" + str({wait_name}).to_utf8_buffer().hex_encode(), __any_won_{uid}[0])\nset_meta(&\"__ef_wait_\" + str({wait_name}).to_utf8_buffer().hex_encode(), 1 if __any_won_{uid}[0] != \"\" else 2)\nif __any_won_{uid}[0] == \"\" and has_signal(&\"verb_failed\"):\n\temit_signal(&\"verb_failed\", str({wait_name}), \"timed out after \" + str({seconds}) + \"s\")", "", [F.make_param("wait_name", "String", "\"race\"", "Named", "A name for this wait, so First To Finish and the outcome conditions can say which one they mean.", "expression"), F.make_param("signals", "String", "[]", "Signals", "The signals to race, as a list: [$Player.died, $Boss.died].", "expression"), F.make_param("seconds", "String", "0.0", "Give Up After", "Seconds before giving up. 0 waits forever.", "expression")], "Time", "wait [b]{wait_name}[/b] for ANY of [b]{signals}[/b]")
		.described("Pauses this event until the first of several signals fires, then carries on - a race, or a sudden-death timer. Read which one won with First To Finish, naming this wait: it answers \"Node.signal\", so racing $Player.died against $Boss.died still tells you which one it was. Every signal is connected one-shot up front, so the winner is whichever really fired first, and the losers are disconnected when the race ends."))
	descriptors.append(F.make_descriptor("Core", "WaitSucceeded", "Wait Succeeded", ACEDescriptor.ACEType.CONDITION, "int(get_meta(&\"__ef_wait_\" + str({wait_name}).to_utf8_buffer().hex_encode(), 0)) == 1", "", [F.make_param("wait_name", "String", "\"loading\"", "Named", "The name the wait row used.", "expression")], "Run Context", "wait [b]{wait_name}[/b] succeeded")
		.described("True when the named wait ended because what it was waiting for happened. A wait that has never run is neither succeeded nor timed out, so nothing fires before the wait does."))
	descriptors.append(F.make_descriptor("Core", "WaitTimedOut", "Wait Timed Out", ACEDescriptor.ACEType.CONDITION, "int(get_meta(&\"__ef_wait_\" + str({wait_name}).to_utf8_buffer().hex_encode(), 0)) == 2", "", [F.make_param("wait_name", "String", "\"loading\"", "Named", "The name the wait row used.", "expression")], "Run Context", "wait [b]{wait_name}[/b] timed out")
		.described("True when the named wait gave up instead of finishing - the recovery branch for a load that never reported in. The same timeout also reports on verb_failed, so another event can handle it as On Failure Of."))
	descriptors.append(F.make_descriptor("Core", "FirstToFinish", "First To Finish", ACEDescriptor.ACEType.EXPRESSION, "str(get_meta(&\"__ef_first_\" + str({wait_name}).to_utf8_buffer().hex_encode(), \"\"))", "", [F.make_param("wait_name", "String", "\"race\"", "Named", "The name the Wait For Any Of row used.", "expression")], "Run Context", "first to finish ( [b]{wait_name}[/b] )")
		.described("Which signal a Wait For Any Of was watching finished first, written as \"Node.signal\" (the node's name, a dot, the signal's name), or empty text when it timed out. The node is part of the answer on purpose: a race between $Player.died and $Boss.died is two signals with the same name, and only the owner tells them apart. Compare it in a condition cell to branch on the winner."))
	...
	descriptors.append(F.make_descriptor("Core", "RetryUpTo", "Retry Up To N Times", ACEDescriptor.ACEType.CONDITION, "__retry_begin_{uid}(str({retry_name}), int({times}))", "", [F.make_param("retry_name", "String", "\"save\"", "Named", "A name for this retry, matching its Stop Retrying and Retries Exhausted rows.", "expression"), F.make_param("times", "String", "5", "Times", "Most attempts before giving up.", "expression")], "Loops", "retry [b]{retry_name}[/b] up to [b]{times}[/b] times")
		.described("Runs this event's actions over and over, up to a number of attempts, so a nested Stop Retrying can end it the moment the thing works. With no waiting inside it stays instant, which makes it the procedural-placement loop as much as the disk retry. Read the try number with Retry Attempt Number.")
		.looping("attempt")
		.stateful("func __retry_begin_{uid}(key: String, times: int) -> Array:\n\tset_meta(StringName(\"__ef_retry_\" + key.to_utf8_buffer().hex_encode()), 1)\n\treturn range(maxi(times, 1))"))
	descriptors.append(F.make_descriptor("Core", "RetryAttemptNumber", "Retry Attempt Number", ACEDescriptor.ACEType.EXPRESSION, "({loop_var} + 1)", "", [F.make_param("loop_var", "String", "attempt", "Loop Variable", "The retry loop's own variable. Leave it as attempt unless you renamed the loop variable on the Retry Up To N Times row.", "expression")], "Loops", "retry attempt number")
		.described("Which try this is, counting from 1 - the number you put in the log line or the HUD. Only readable inside a Retry Up To N Times loop, and it reads that loop's own variable, so change the cell if you renamed it."))
	descriptors.append(F.make_descriptor("Core", "StopRetrying", "Stop Retrying", ACEDescriptor.ACEType.ACTION, "set_meta(&\"__ef_retry_\" + str({retry_name}).to_utf8_buffer().hex_encode(), 2)\nbreak", "", [F.make_param("retry_name", "String", "\"save\"", "Name", "The name the Retry Up To N Times row uses.", "expression")], "Loops", "stop retrying [b]{retry_name}[/b]")
		.described("Ends the retry loop right now because the attempt worked, and records that it worked so Retries Exhausted below stays false. Put it under the condition that means success."))
	descriptors.append(F.make_descriptor("Core", "RetriesExhausted", "Retries Exhausted", ACEDescriptor.ACEType.CONDITION, "__exhausted_{uid}(str({retry_name}))", "", [F.make_param("retry_name", "String", "\"save\"", "Name", "The name the Retry Up To N Times row uses.", "expression")], "Loops", "retries for [b]{retry_name}[/b] exhausted")
		.described("True when the retry loop above ran out of tries without a Stop Retrying - the give-up branch. Put it in a sibling row directly beneath the retry loop, using the same name; a retry that has never run is not exhausted, so nothing fires before the loop does. Reading it clears the record, so the next run starts clean. When the retry WAITS between tries (Wait Before Next Try) the loop is still running while the rows beneath it are reached, so report that give-up with Report Failure on the last attempt and handle it in an On Failure Of event instead.")
		.stateful("func __exhausted_{uid}(key: String) -> bool:\n\tvar meta_key: StringName = StringName(\"__ef_retry_\" + key.to_utf8_buffer().hex_encode())\n\tvar state: int = int(get_meta(meta_key, 0))\n\tset_meta(meta_key, 0)\n\treturn state == 1"))
	descriptors.append(F.make_descriptor("Core", "WaitBeforeNextTry", "Wait Before Next Try", ACEDescriptor.ACEType.ACTION, "await get_tree().create_timer(maxf({delay}, 0.0) * pow(maxf({growth}, 1.0), maxf(float({attempt}) - 1.0, 0.0))).timeout", "", [F.make_param("delay", "String", "0.5", "Seconds", "Wait after the first failed try.", "expression"), F.make_param("growth", "String", "2.0", "Growing By", "1 = the same wait every time, 2 = doubling backoff.", "expression"), F.make_param("attempt", "String", "1", "On Try", "Which try this is - put Retry Attempt Number here.", "expression")], "Loops", "wait [b]{delay}[/b]s before try [b]{attempt}[/b] (growing x[b]{growth}[/b])")
		.described("Pauses before the next attempt, waiting longer each time when you set a growth above 1 - the polite way to retry a disk or a slow source. It suspends this event like Wait does, so pair the retry with Once At A Time when the trigger can fire again mid-retry."))
	# Input expressions
	...
	descriptors.append(F.make_descriptor("Core", "SetSpriteFrame", "Set Animation Frame", ACEDescriptor.ACEType.ACTION, "frame = {frame}", "", [F.make_param("frame", "String", "0", "Frame", "Frame index.", "expression")], "General Actions", "Set animation frame to {frame}", "AnimatedSprite2D")
	...
	descriptors.append(F.make_descriptor("Core", "MakeCameraCurrent", "Make Current", ACEDescriptor.ACEType.ACTION, "make_current()", "", [], "Camera", "Make current", "Camera2D")
		.described("Makes this camera the active one the player views the game through."))
	descriptors.append(F.make_descriptor("Core", "SetCameraZoom", "Set Zoom", ACEDescriptor.ACEType.ACTION, "zoom = {zoom}", "", [F.make_param("zoom", "String", "Vector2(1, 1)", "Zoom", "Zoom factor.", "expression")], "Camera", "Set zoom to {zoom}", "Camera2D")
		.described("Sets how zoomed in or out the camera is."))
	descriptors.append(F.make_descriptor("Core", "SetCameraOffset", "Set Offset", ACEDescriptor.ACEType.ACTION, "offset = {offset}", "", [F.make_param("offset", "String", "Vector2(0, 0)", "Offset", "Offset from the followed position.", "expression")], "Camera", "Set offset to {offset}", "Camera2D")
		.described("Shifts the camera view away from the position it follows."))
	descriptors.append(F.make_descriptor("Core", "SetCameraLimits", "Set Scroll Limits", ACEDescriptor.ACEType.ACTION, "limit_left = {left}\nlimit_top = {top}\nlimit_right = {right}\nlimit_bottom = {bottom}", "", [F.make_param("left", "String", "0", "Left", "Left bound (px).", "expression"), F.make_param("top", "String", "0", "Top", "Top bound (px).", "expression"), F.make_param("right", "String", "1920", "Right", "Right bound (px).", "expression"), F.make_param("bottom", "String", "1080", "Bottom", "Bottom bound (px).", "expression")], "Camera", "Set scroll limits {left} to {right}", "Camera2D")
		.described("Sets the boundaries the camera won't scroll past, keeping it inside the level."))
	descriptors.append(F.make_descriptor("Core", "SetCameraSmoothing", "Set Smoothing", ACEDescriptor.ACEType.ACTION, "position_smoothing_enabled = {enabled}", "", [F.make_param("enabled", "String", "true", "Smoothing", "true / false.", "", ["true", "false"])], "Camera", "Set smoothing {enabled}", "Camera2D")
		.described("Turns the camera's smooth catch-up on or off, so it eases toward what it follows instead of snapping."))
	descriptors.append(F.make_descriptor("Core", "CameraScrollToward", "Scroll Toward", ACEDescriptor.ACEType.ACTION, "global_position = global_position.lerp({toward}.global_position, {rate} * get_process_delta_time())", "", [F.make_param("toward", "String", "self", "Toward", "The node to scroll toward.", "expression"), F.make_param("rate", "String", "5", "Rate", "How fast to close the gap, per second.", "expression")], "Camera", "Scroll toward {toward} at {rate}", "Camera2D")
		.described("Eases the camera toward another node, closing the gap at the given rate every second."))
	...
	descriptors.append(F.make_descriptor("Core", "ToggleVar", "Toggle boolean", ACEDescriptor.ACEType.ACTION, "{var_name} = not {var_name}", "", [F.make_param("var_name", "String", "enabled_flag", "Variable", "Boolean variable to flip.", "variable_reference")], "Variables", "Toggle {var_name}")
	...
	descriptors.append(F.make_descriptor("Core", "ArrayPushFront", "Push Front", ACEDescriptor.ACEType.ACTION, "{var_name}.push_front({value})", "", [F.make_param("var_name", "String", "list", "Array", "Array variable.", "variable_reference:Array"), F.make_param("value", "String", "0", "Value", "Value to prepend.", "expression")], "Variables: Array", "Push front {value} to {var_name}")
		.described("Inserts a value at the start of the array, shifting the rest along."))
	descriptors.append(F.make_descriptor("Core", "ArrayPopBack", "Pop Back", ACEDescriptor.ACEType.EXPRESSION, "{var_name}.pop_back()", "", [F.make_param("var_name", "String", "list", "Array", "Array variable (removes + returns last).", "variable_reference:Array")], "Variables: Array", "{var_name}.pop_back()")
		.described("Removes and returns the last item of the array."))
	descriptors.append(F.make_descriptor("Core", "ArrayPopFront", "Pop Front", ACEDescriptor.ACEType.EXPRESSION, "{var_name}.pop_front()", "", [F.make_param("var_name", "String", "list", "Array", "Array variable (removes + returns first).", "variable_reference:Array")], "Variables: Array", "{var_name}.pop_front()")
