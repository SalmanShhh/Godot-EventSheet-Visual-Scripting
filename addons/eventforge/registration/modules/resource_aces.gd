...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(F.expr("ResourcesInFolder", "Resources In Folder", "Array(DirAccess.get_files_at({folder}) if DirAccess.dir_exists_absolute({folder}) else PackedStringArray()).map(func(__f): return {folder}.path_join(__f.trim_suffix(\".remap\"))).filter(func(__p): return __p.get_extension() in [\"tres\", \"res\"]).map(func(__p): return load(__p)).filter(func(__resource): return __resource != null)", CAT_FILES, "resources in [b]{folder}[/b]", "Loads every data asset (.tres) in a folder as a list, so a folder of files becomes your content - items, enemies, levels, or a mod folder. A missing folder gives an empty list, and a file that fails to load is left out rather than arriving as nothing.").param("folder", "\"res://data/items\"", "Folder", "Folder to scan (not recursive). Every .tres / .res inside it is loaded, in the folder's own order.", "expression"))
	descriptors.append(F.expr("ResourceInFolder", "Resource In Folder", "(load({folder}.path_join({name}) + \".tres\") if ResourceLoader.exists({folder}.path_join({name}) + \".tres\") else null)", CAT_FILES, "resource [b]{name}[/b] in [b]{folder}[/b]", "Fetches one data asset out of a folder by its file name, or nothing at all when there is no such file - no red error.").param("folder", "\"res://data/items\"", "Folder", "Folder holding the data assets.", "expression").param("name", "\"item\"", "Name", "File name WITHOUT the .tres extension - \"rusty_sword\" finds rusty_sword.tres.", "expression"))
	descriptors.append(F.expr("LoadResourceOrDefault", "Load Resource Or Default", "(load({path}) if ResourceLoader.exists({path}) else {fallback})", CAT_FILES, "load [b]{path}[/b] or [i]{fallback}[/i]", "Loads a file and hands back your fallback when it is missing, so a deleted or mod-supplied file never crashes the game.").param("path", "\"res://data/item.tres\"", "Path", "Full path to the resource or scene to load.", "expression").param("fallback", "null", "Fallback", "What to use instead when the file is not there (a preloaded default, or null).", "expression"))
	...
	# be missing.
	descriptors.append(F.expr("DataAsset", "Data Asset", "load({path})", CAT_FILES, "the data asset [b]{path}[/b]", "Fetches one data asset by its path - the values a designer filled in the Inspector, ready to read fields off.").param("path", "\"res://data/item.tres\"", "File", "The .tres to fetch.", "expression").featured())
	descriptors.append(F.act("SaveDataAsset", "Save Data Asset", "ResourceSaver.save({resource}, {path})", CAT_FILES, "Save data asset [b]{resource}[/b] as [b]{path}[/b]", "Writes a data asset back to a file. An editor tool uses this to generate or bulk-edit content; a running game should write under user://.").param("resource", "null", "Asset", "The data asset to write out.", "expression").param("path", "\"res://data/item.tres\"", "As", "Where to write it. Writing under res:// only works in the editor; a running game writes to user://.", "expression"))
	descriptors.append(F.expr("CountResourcesInFolder", "Count Of Resources In", "Array(DirAccess.get_files_at({folder}) if DirAccess.dir_exists_absolute({folder}) else PackedStringArray()).filter(func(__f): return __f.trim_suffix(\".remap\").get_extension() in [\"tres\", \"res\"]).size()", CAT_FILES, "count of resources in [b]{folder}[/b]", "How many data assets a folder holds, counted without loading any of them - zero if the folder is missing.").param("folder", "\"res://data/items\"", "Folder", "Folder to count data assets in (not recursive).", "expression"))
	...
	descriptors.append(F.expr("CopyResourceDeep", "Copy Resource (Independent)", "({resource}.duplicate(true) if {resource} is Resource else null)", CAT_HELPERS, "independent copy of [i]{resource}[/i]", "Makes a private copy of a resource, right down to the resources inside it, so writing to the copy never changes the .tres on disk or any other node holding it. Use this before a node edits its own stats.").param("resource", "Resource.new()", "Resource", "The resource to copy. Anything that is not a resource (including nothing at all) gives nothing back.", "expression"))
	descriptors.append(F.expr("CopyResourceShallow", "Copy Resource (Share Sub-Resources)", "({resource}.duplicate(false) if {resource} is Resource else null)", CAT_HELPERS, "copy of [i]{resource}[/i] (sub-resources shared)", "Makes a cheap copy whose own fields are separate but whose inner resources and lists are still SHARED with the original. Pick this only when you want that sharing - otherwise use Copy Resource (Independent).").param("resource", "Resource.new()", "Resource", "The resource to copy. Anything that is not a resource (including nothing at all) gives nothing back.", "expression"))
	descriptors.append(F.expr("ArrayDeepDuplicate", "Deep Copy", "{var_name}.duplicate(true)", CAT_ARRAY, "deep copy of [b]{var_name}[/b]", "Copies the array AND every list or dictionary nested inside it, so editing the copy cannot reach back into the original. Copy Array only copies the outer level.").param("var_name", "list", "Array", "Array variable to copy right through.", "variable_reference:Array"))
	descriptors.append(F.expr("DictDeepDuplicate", "Deep Copy", "{var_name}.duplicate(true)", CAT_DICT, "deep copy of [b]{var_name}[/b]", "Copies the dictionary AND every list or dictionary nested inside it, so editing the copy cannot reach back into the original. Copy Dictionary only copies the outer level.").param("var_name", "dict", "Dictionary", "Dictionary variable to copy right through.", "variable_reference:Dictionary"))

	# ── Helpers - pouring values between objects (no cloning, no six-row mirror) ──
	descriptors.append(F.act("CopyValuesFrom", "Copy Values From", _COPY_VALUES_TEMPLATE, CAT_HELPERS, "copy [b]{names}[/b] from [i]{source}[/i] into [i]{target}[/i]", "Pours a list of values off another object onto this one, in one row instead of one row per property. Names the target does not have are skipped, so a single preset can serve several kinds of node.").param("target", "self", "Into", "The object being written to - it is what changes.", "expression").param("source", "self", "From", "The object to read from - a node OR a resource.", "expression").param("names", "\"\"", "Names", "Comma-separated property names. Leave blank to copy every variable the source's script declares.", "expression"))
	descriptors.append(F.act("FillBlanksFrom", "Fill Blanks From", _FILL_BLANKS_TEMPLATE, CAT_HELPERS, "fill blanks in [i]{target}[/i] from [i]{base}[/i]", "Writes a base's values ONLY into fields this object left empty, leaving everything you did fill in alone. That is the override chain: a base item plus a rarity variant, or a shipped table plus a mod file. Empty means nothing there: no value at all, blank text, an empty list or an empty record - a 0 and a false are real values and are kept, exactly as Is Nothing and Missing Fields read them.").param("target", "self", "Fill", "The one being completed - it is what changes.", "expression").param("base", "self", "From", "Where the missing values come from.", "expression"))
	descriptors.append(F.act("ApplyPresetToNode", "Apply Preset To Node", _APPLY_PRESET_TEMPLATE, CAT_HELPERS, "apply preset [b]{preset}[/b] to [i]{target}[/i]", "Pours a data asset's fields onto the same-named properties of a node, so difficulty tiers, weapon tunings and boss phases become a data edit instead of a wall of rows.").param("preset", "null", "Preset", "The data asset to pour on (a .tres you filled in the Inspector). Nothing happens if it is empty.", "expression").param("target", "self", "To", "The node to apply it to.", "expression"))
	...
	descriptors.append(F.cond("MatchesPropertiesOf", "Matches Properties Of", "({target} != null and {other} != null and Array({names}.split(\",\", false)).all(func(__n): return __n.strip_edges() in {target} and {target}.get(__n.strip_edges()) == {other}.get(__n.strip_edges())))", CAT_HELPERS, "[i]{target}[/i] matches [b]{names}[/b] of [i]{other}[/i]", "True while the listed properties hold the same values on both objects - the cheap 'is this ghost still in sync' or 'has anything changed' check. A name neither object has reads as NOT matching, so a typo shows up instead of quietly passing, and either side being gone reads as not matching too.").param("target", "self", "Object", "The object to test.", "expression").param("names", "\"position, rotation\"", "Names", "Comma-separated property names to compare. A blank list has nothing to compare, so it is true.", "expression").param("other", "self", "Of", "The object to compare against.", "expression"))
	...
	descriptors.append(F.cond("DataIsOlderThanVersion", "Data Is Older Than Version", "(str({record}.get({field}, 0)).to_int() < {version})", CAT_DICT, "[b]{record}[/b] is older than version [b]{version}[/b]", "True when a loaded record was written by an older build than this one - the gate a migration sits under. A record with NO version field counts as 0, so the very first format upgrades too, and so does one whose version field is empty or is not a number at all.").param("record", "save", "Record", "The parsed record (a save file, a payload, a manifest).", "variable_reference:Dictionary").param("field", "\"version\"", "Field", "Which field holds the version number.", "expression").param("version", "2", "Version", "The version this build writes.", "expression"))
	descriptors.append(F.act("RenameField", "Rename Field", _RENAME_FIELD_TEMPLATE, CAT_DICT, "rename [b]{from_field}[/b] to [b]{to_field}[/b] in [i]{record}[/i]", "Moves a value to its new field name, and does nothing at all when the old name is not there - so a migration is safe to run twice.").param("record", "save", "Record", "The record to migrate, edited in place.", "variable_reference:Dictionary").param("from_field", "\"hp\"", "From", "The old field name.", "expression").param("to_field", "\"health\"", "To", "The new field name (overwritten if it is already there).", "expression"))
	...
	descriptors.append(F.act("StampDataVersion", "Stamp Data Version", "{record}[{field}] = {version}", CAT_DICT, "stamp [i]{record}[/i] as version [b]{version}[/b]", "Writes the current format number onto a record, so the next load knows it has already been migrated. Data Is Older Than Version reads it back, and reads it as 0 when it is missing or is not a number.").param("record", "save", "Record", "The record to stamp.", "variable_reference:Dictionary").param("field", "\"version\"", "Field", "Which field holds the version number.", "expression").param("version", "2", "Version", "The version this build writes - a whole number.", "expression"))
	...
	descriptors.append(F.act("WatchDataFile", "Watch Data File", "var __stamp_{uid}: int = FileAccess.get_modified_time({path}) if FileAccess.file_exists({path}) else 0\nif __stamp_{uid} != int(get_meta(&\"__ef_watch_\" + str({path}).to_utf8_buffer().hex_encode(), __stamp_{uid})):\n\tif has_signal(&\"data_file_changed\"):\n\t\temit_signal(&\"data_file_changed\", {path})\nset_meta(&\"__ef_watch_\" + str({path}).to_utf8_buffer().hex_encode(), __stamp_{uid})", CAT_FILES, "watch [b]{path}[/b] for edits", "Checks whether a data file has been written since the last check, and fires the sheet's data_file_changed(path) signal when it has - so you edit an enemy's numbers in the Inspector and the running game picks them up. Put it under Every X Seconds; the first check only takes a reading, so nothing fires just because the row started. This is a debug-build tool: it reads the file's timestamp each time it runs.").param("path", "\"res://data/items/sword.tres\"", "File", "The data file to watch (a .tres, .json or .csv).", "expression"))
	descriptors.append(F.act("ReloadDataAsset", "Reload Data Asset", "if ResourceLoader.exists({path}):\n\tResourceLoader.load({path}, \"\", ResourceLoader.CACHE_MODE_REPLACE)", CAT_FILES, "reload data asset [b]{path}[/b]", "Re-reads a data asset from disk into the copy every node is already holding, so the new numbers apply without restarting or re-assigning anything. It reloads DATA, not code: a changed script still needs a restart. Nothing happens when the path is not there.").param("path", "\"res://data/items/sword.tres\"", "File", "The data asset to re-read from disk.", "expression"))
	...
	descriptors.append(F.trig("signal:data_file_changed", "On Data File Changed", "data_file_changed", CAT_FILES, "On data file changed ( {path} )", "Runs when a Watch Data File row notices that a watched file has been written. The path arrives on the row as path, so the reaction reloads exactly the file that changed even when two land in the same check. Needs a Signal row for data_file_changed(path: String) - without one the sheet still compiles, but nothing connects this event, so it never runs. The Project Doctor flags that.").param("path", "", "File", "The file that was written, carried by the signal itself."))
	descriptors.append(F.expr("DataFolderProblems", "Data Folder Problems", folder_problems, CAT_FILES, "problems in [b]{folder}[/b]", "Every structural problem in a folder of data assets, one per line, and \"\" when it is clean: a file that cannot be loaded, one with no usable id, and two files claiming the same id (where the second quietly wins every lookup). Show it, log it, or fail a build with it.").param("folder", "\"res://data/items\"", "Folder", "Folder of data assets to check (not recursive).", "expression"))
	descriptors.append(F.cond("DataFolderIsValid", "Data Folder Is Valid", "(%s).is_empty()" % folder_problems, CAT_FILES, "[b]{folder}[/b] is valid data", "True when every data asset in a folder loads, has an id, and has an id no sibling shares - the check to put in front of loading a mod folder or user content. Read the reasons with Data Folder Problems.").param("folder", "\"res://data/items\"", "Folder", "Folder of data assets to check (not recursive).", "expression"))
	descriptors.append(F.act("ValidateDataFolder", "Validate Data Folder", "var __report_{uid}: String = %s\nif not __report_{uid}.is_empty():\n\tpush_warning(\"Data folder \" + {folder} + \" has problems:\\n\" + __report_{uid})" % folder_problems, CAT_FILES, "validate data folder [b]{folder}[/b]", "Checks a folder of data assets and writes every problem it finds to the Output as one warning, saying which file and what is wrong. A clean folder says nothing at all, so it is safe to leave in a startup event.").param("folder", "\"res://data/items\"", "Folder", "Folder of data assets to check (not recursive).", "expression"))
