...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(F.make_descriptor("Core", "GetMouseVelocity", "Mouse Velocity", ACEDescriptor.ACEType.EXPRESSION, "Input.get_last_mouse_velocity()", "", [], "Mouse", "mouse velocity")
		.described("How fast the mouse is moving (a Vector2 in pixels per second) - flick gestures, aim sway, spin-the-wheel minigames."))
	descriptors.append(F.make_descriptor("Core", "IsMouseCaptured", "Mouse Is Captured", ACEDescriptor.ACEType.CONDITION, "Input.mouse_mode == Input.MOUSE_MODE_CAPTURED", "", [], "Mouse", "mouse is captured")
		.described("True while the cursor is locked to the window (the FPS look mode) - gate pause menus and hints on it."))
	descriptors.append(F.make_descriptor("Core", "MouseButtonEventPressed", "On Mouse Button Pressed (event)", ACEDescriptor.ACEType.CONDITION, "(event is InputEventMouseButton and event.pressed and event.button_index == {button})", "", [F.make_param("button", "String", "MOUSE_BUTTON_LEFT", "Button", "Mouse button.", "", ["MOUSE_BUTTON_LEFT", "MOUSE_BUTTON_RIGHT", "MOUSE_BUTTON_MIDDLE"])], "Mouse", "On {button} pressed")
		.described("True the moment a mouse button goes down, used inside an On Input event (the click-y partner of Mouse Button Is Down)."))
	descriptors.append(F.make_descriptor("Core", "MouseButtonEventReleased", "On Mouse Button Released (event)", ACEDescriptor.ACEType.CONDITION, "(event is InputEventMouseButton and not event.pressed and event.button_index == {button})", "", [F.make_param("button", "String", "MOUSE_BUTTON_LEFT", "Button", "Mouse button.", "", ["MOUSE_BUTTON_LEFT", "MOUSE_BUTTON_RIGHT", "MOUSE_BUTTON_MIDDLE"])], "Mouse", "On {button} released")
		.described("True the moment a mouse button is let go, used inside an On Input event."))
	descriptors.append(F.make_descriptor("Core", "MouseWheelUpEvent", "On Mouse Wheel Up (event)", ACEDescriptor.ACEType.CONDITION, "(event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_WHEEL_UP)", "", [], "Mouse", "On mouse wheel up")
		.described("True on a wheel-up tick, used inside an On Input event - zoom in, next weapon, scroll a list."))
	descriptors.append(F.make_descriptor("Core", "MouseWheelDownEvent", "On Mouse Wheel Down (event)", ACEDescriptor.ACEType.CONDITION, "(event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_WHEEL_DOWN)", "", [], "Mouse", "On mouse wheel down")
		.described("True on a wheel-down tick, used inside an On Input event."))
	descriptors.append(F.make_descriptor("Core", "SetCustomCursor", "Set Custom Cursor", ACEDescriptor.ACEType.ACTION, "Input.set_custom_mouse_cursor(load({image_path}))", "", [F.make_param("image_path", "String", "\"res://cursor.png\"", "Image", "Path to the cursor image (png).", "expression")], "Mouse", "Set cursor to {image_path}")
		.described("Swaps the mouse pointer for your own image - a crosshair, a sword, a hand."))
	descriptors.append(F.make_descriptor("Core", "ClearCustomCursor", "Clear Custom Cursor", ACEDescriptor.ACEType.ACTION, "Input.set_custom_mouse_cursor(null)", "", [], "Mouse", "Clear custom cursor")
		.described("Restores the system's normal mouse pointer."))

	# ── Keyboard helpers for options menus and rebind screens.
	descriptors.append(F.make_descriptor("Core", "IsAnythingPressed", "Anything Is Pressed", ACEDescriptor.ACEType.CONDITION, "Input.is_anything_pressed()", "", [], "Keyboard", "any input is pressed")
		.described("True while ANY key, mouse button, or gamepad input is held - press-any-key screens and idle detection."))
	descriptors.append(F.make_descriptor("Core", "KeyName", "Key Name", ACEDescriptor.ACEType.EXPRESSION, "OS.get_keycode_string({key})", "", [F.make_param("key", "String", "KEY_SPACE", "Key", "Press a key to capture it.", "key_capture")], "Keyboard", "name of key {key}")
		.described("The readable name of a key (\"Space\", \"Escape\") - show current bindings on a rebind screen."))
	descriptors.append(F.make_descriptor("Core", "KeycodeFromName", "Keycode From Name", ACEDescriptor.ACEType.EXPRESSION, "OS.find_keycode_from_string({name})", "", [F.make_param("name", "String", "\"Space\"", "Name", "A key name like \"Space\" or \"A\".", "expression")], "Keyboard", "keycode of {name}")
		.described("The keycode for a key name - turn saved binding text back into a key."))

	# ── Gamepad helpers for detection and UI.
	descriptors.append(F.make_descriptor("Core", "GamepadCount", "Gamepad Count", ACEDescriptor.ACEType.EXPRESSION, "Input.get_connected_joypads().size()", "", [], "Gamepad", "connected gamepad count")
		.described("How many gamepads are plugged in - local-multiplayer joins and control-hint switching."))
	descriptors.append(F.make_descriptor("Core", "GamepadName", "Gamepad Name", ACEDescriptor.ACEType.EXPRESSION, "Input.get_joy_name({device})", "", [F.make_param("device", "String", "0", "Gamepad", "Device index (0 = first).", "expression")], "Gamepad", "gamepad {device} name")
		.described("The connected gamepad's product name (\"Xbox Series Controller\") - show the right button glyphs."))
	descriptors.append(F.make_descriptor("Core", "GamepadIsKnown", "Gamepad Is Recognized", ACEDescriptor.ACEType.CONDITION, "Input.is_joy_known({device})", "", [F.make_param("device", "String", "0", "Gamepad", "Device index.", "expression")], "Gamepad", "gamepad {device} is recognized")
		.described("True when the gamepad matches a known mapping (its buttons mean what you expect)."))
	descriptors.append(F.make_descriptor("Core", "JoyButtonEventPressed", "On Gamepad Button Pressed (event)", ACEDescriptor.ACEType.CONDITION, "(event is InputEventJoypadButton and event.pressed and event.button_index == {button})", "", [F.make_param("button", "String", "JOY_BUTTON_A", "Button", "Gamepad button.", "", ["JOY_BUTTON_A", "JOY_BUTTON_B", "JOY_BUTTON_X", "JOY_BUTTON_Y", "JOY_BUTTON_LEFT_SHOULDER", "JOY_BUTTON_RIGHT_SHOULDER", "JOY_BUTTON_START", "JOY_BUTTON_BACK", "JOY_BUTTON_DPAD_UP", "JOY_BUTTON_DPAD_DOWN", "JOY_BUTTON_DPAD_LEFT", "JOY_BUTTON_DPAD_RIGHT"])], "Gamepad", "On gamepad {button} pressed")
		.described("True the moment a gamepad button goes down, used inside an On Input event."))
	...
	descriptors.append(F.make_descriptor("Core", "ActionAddEvent", "Bind Event To Action", ACEDescriptor.ACEType.ACTION, "InputMap.action_add_event(&{action}, {event})", "", [F.make_param("action", "String", F.default_input_action(), "Action", "Input action to bind to.", "input_action", F.input_action_options()), F.make_param("event", "String", "event", "Event", "InputEvent to add (e.g. the captured `event`).", "expression")], "InputMap", "Bind {event} to {action}")
		.described("Binds a new key, button, or input to a named action at runtime, for remapping."))
	descriptors.append(F.make_descriptor("Core", "ActionEraseEvents", "Clear Action Bindings", ACEDescriptor.ACEType.ACTION, "InputMap.action_erase_events(&{action})", "", [F.make_param("action", "String", F.default_input_action(), "Action", "Input action to clear.", "input_action", F.input_action_options())], "InputMap", "Clear bindings for {action}")
		.described("Removes all key and button bindings from a named input action."))
	descriptors.append(F.make_descriptor("Core", "ActionHasEvents", "Action Is Bound", ACEDescriptor.ACEType.CONDITION, "not InputMap.action_get_events(&{action}).is_empty()", "", [F.make_param("action", "String", F.default_input_action(), "Action", "Input action to test.", "input_action", F.input_action_options())], "InputMap", "{action} is bound")
		.described("True when the named input action has at least one key or button bound."))
	descriptors.append(F.make_descriptor("Core", "ActionEventCount", "Action Binding Count", ACEDescriptor.ACEType.EXPRESSION, "InputMap.action_get_events(&{action}).size()", "", [F.make_param("action", "String", F.default_input_action(), "Action", "Input action.", "input_action", F.input_action_options())], "InputMap", "{action} binding count")
		.described("Returns how many keys or buttons are bound to a named action."))
	descriptors.append(F.make_descriptor("Core", "EventMatchesAction", "Event Matches Action", ACEDescriptor.ACEType.CONDITION, "{event}.is_action(&{action})", "", [F.make_param("event", "String", "event", "Event", "InputEvent to test.", "expression"), F.make_param("action", "String", F.default_input_action(), "Action", "Input action.", "input_action", F.input_action_options())], "InputMap", "{event} matches {action}")
