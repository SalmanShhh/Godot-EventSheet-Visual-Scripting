...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	#
	# The up vector is swapped the same way the safe-up Look At swaps it: an aim within a thousandth
	# of straight up or straight down is parallel to Vector3.UP, and Basis.looking_at cannot build a
	# rotation out of two parallel vectors - it warns and hands back an arbitrary roll. The
	# length guard above it answers a different question (a target the camera is standing on has no
	# direction at all), and one does not cover the other.
	descriptors.append(F.act("CameraLookAtOverSeconds", "Look At Over Seconds", "var __aim_{uid}: Vector3 = {at}.global_position - global_position\nif __aim_{uid}.length_squared() > 0.000001:\n\tvar __from_{uid}: Basis = global_basis\n\tvar __to_{uid}: Basis = Basis.looking_at(__aim_{uid}, Vector3.UP if absf(__aim_{uid}.normalized().y) < 0.999 else Vector3.FORWARD)\n\tcreate_tween().tween_method(func(__weight_{uid}: float) -> void: global_basis = __from_{uid}.slerp(__to_{uid}, __weight_{uid}), 0.0, 1.0, maxf({seconds}, 0.001))", CAT, "Look at [i]{at}[/i] over {seconds}s", "Turns the camera to face a node over time instead of snapping to it - the difference between a cut and a shot. It walks the shortest rotation between where the camera is looking and where it should look, so it never rolls sideways on the way, and it swaps the up vector at the last moment when the target is straight overhead or underfoot - the top-down shot that would otherwise hand back an arbitrary roll. The row owns the camera's orientation for the whole of those seconds: a second turn started before this one lands, or a camera the player is also steering, will fight it. A target sitting exactly where the camera is has no direction to face, so the row does nothing at all rather than erroring.", "Camera3D").param("at", "$Player", "Look At", "The node to turn towards.", "scene_node").param_typed("float", "seconds", "0.6", "Over", "How long the turn takes.", "expression").featured())
