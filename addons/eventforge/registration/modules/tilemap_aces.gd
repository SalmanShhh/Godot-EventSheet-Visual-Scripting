...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	# spells over two lines, in the one condition the sheet has for it. It names its layer in an
	# "On node" parameter of its own, like the helper rows below and for the same reason: the
	# automatic {target.} prefix goes on the START of a line only, so a template naming the layer
	# TWICE would retarget the guard and read the lookup off the host - half a row pointed at one
	# node and half at another.
	descriptors.append(F.cond("TileMapTileHasCustomData", "Tile Has Custom Data", "{target}.get_cell_tile_data({coords}) != null and {target}.get_cell_tile_data({coords}).get_custom_data({name})", "Tilemap", "tile at {coords} has {name} set", "True when the tile at a cell carries the named custom data - how a tileset marks walls, water or ladders.", "TileMapLayer").param("target", "self", "On node", ON_NODE_WORDS, "expression").param("coords", "Vector2i(0, 0)", "Cell", "Cell coordinates (Vector2i).", "expression").param("name", "\"solid\"", "Data", "The custom data layer's name."))
...
static func _add_layer_files(descriptors: Array[ACEDescriptor]) -> void:
	...
	descriptors.append(F.act("TileMapLoadLayer", "Load Layer From File", "%s({target}, {path})" % LOAD_LAYER, "Tilemap", "Load layer from {path}", "Puts the tiles from a file back onto the layer, replacing whatever it held. A file that is not there leaves the layer alone rather than clearing it. The file holds tile NUMBERS, not pictures: one written from a layer with a different tileset fills cells with ids this tileset has nothing to draw, and nothing errors - save the tileset with the level, or write your own version number beside it.", "TileMapLayer").param("target", "self", "On node", ON_NODE_WORDS, "expression").param("path", "\"user://levels/mine.tiles\"", "File", "The file to read. user:// is the player's own folder.", "file_path"))
