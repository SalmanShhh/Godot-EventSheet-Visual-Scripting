...
static func _asking(descriptors: Array[ACEDescriptor]) -> void:
	descriptors.append(F.cond("IsOwnedBy", "Is Owned By", "(%s == %s)" % [root_owner_expression("{node}"), root_owner_expression("{owner}")], OWNERSHIP, "[i]{node}[/i] is owned by [i]{owner}[/i]", "True while a node traces back to the owner you name. Both sides are walked to the root, so a bullet fired by a turret the player built counts as the player's - and as the turret's, because the turret traces back to the same person.").param_typed("Node", "node", "self", "Node", "The thing being asked about.", "scene_node").param_typed("Node", "owner", "self", "Owner", "Who it should trace back to.", "expression"))
	descriptors.append(F.cond("IsMine", "Is Mine", "(%s == %s)" % [root_owner_expression("{node}"), root_owner_expression("self")], OWNERSHIP, "[i]{node}[/i] is mine", "True while a node traces back to the same owner this row does - my bullet, my turret, my summon, or me. The friendly half of the pair: Hit Is Not My Owner is the same question asked the other way.").param_typed("Node", "node", "self", "Node", "The thing being asked about.", "scene_node"))
	descriptors.append(F.cond("HitIsNotMyOwner", "Hit Is Not My Owner", "(%s != %s)" % [root_owner_expression("{hit}"), root_owner_expression("self")], OWNERSHIP, "hit [i]{hit}[/i] is not my owner", "The friendly-fire guard, for any hit trigger: true while the thing that was hit does not trace back to whoever fired this. Put the trigger's own collider in the field - the bullet stops shooting the player who fired it, and the turret that player built, without a single flag.").param_suggesting("hit", "self", "Hit", "What was hit - the collider, body or area the trigger above handed you.", ["collider", "body", "area", "hit"] as Array[String], "expression").featured())
