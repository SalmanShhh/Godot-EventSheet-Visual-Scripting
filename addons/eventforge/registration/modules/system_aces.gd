...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(F.make_descriptor("Core", "TokenAt", "Token At", ACEDescriptor.ACEType.EXPRESSION, "{text}.get_slice({separator}, int({index}))", "", [F.make_param("text", "String", "\"a,b,c\"", "Text", "Source string of separated tokens.", "expression"), F.make_param("index", "String", "0", "Index", "Which token to take (0 = first).", "expression"), F.make_param("separator", "String", "\",\"", "Separator", "The string between tokens.", "expression")], "Text", "tokenat({text}, {index}, {separator})")
		.described("Gives one piece of a separated string, like the 2nd name in \"ann,bob,cat\"."))
	descriptors.append(F.make_descriptor("Core", "TokenCount", "Token Count", ACEDescriptor.ACEType.EXPRESSION, "{text}.get_slice_count({separator})", "", [F.make_param("text", "String", "\"a,b,c\"", "Text", "Source string of separated tokens.", "expression"), F.make_param("separator", "String", "\",\"", "Separator", "The string between tokens.", "expression")], "Text", "tokencount({text}, {separator})")
		.described("Gives how many pieces a separated string holds."))
