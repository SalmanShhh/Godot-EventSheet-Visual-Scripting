...
static func get_descriptors() -> Array[ACEDescriptor]:
	...
	descriptors.append(F.cond("AnimationStateIs", "Current State Is", "get(\"parameters/playback\").get_current_node() == {state}", CAT, "Current state is {state}", "True while a blend tree's state machine is in the named state - what a landing recovery or an attack window branches on.", "AnimationTree").param("state", "\"Idle\"", "State", "The name of the state-machine node to ask about. The list is the tree's own states.", "animation_state"))
...
static func _blend_tree(descriptors: Array[ACEDescriptor]) -> void:
	...
	descriptors.append(F.act("BlendToward", "Blend Toward", "create_tween().tween_property({target}, \"parameters/{space}/blend_position\", {value}, {seconds})", CAT, "blend {space} to {value} over {seconds} s", "The same move, taken over time instead of at once - the stick snaps, the blend should not. A tween on the tree's own parameter, so nothing has to be stepped by hand.", "AnimationTree").param("space", "Locomotion", "Blend space", "The blend space node in the tree, by its own name.", "blend_space").param_typed("Vector2", "value", "Vector2.ZERO", "To", "Where the blend ends up.", "expression").param("seconds", "0.2", "Over", "How long the move takes, in seconds.", "expression").param_typed("Node", "target", "self", "On node", "The AnimationTree to blend. Leave it as self when this sheet is the tree.", "scene_node"))
	descriptors.append(F.act("BlendLayer", "Blend Layer", "create_tween().tween_property({target}, \"parameters/{layer}/blend_amount\", {amount}, {seconds})", CAT, "blend layer {layer} to {amount} over {seconds} s", "Mixes a layer in or out over time - the aim pose that fades on when a target is locked, the hurt overlay that fades off. The layer is a Blend2 or an Add2 in the tree, and the amount is 0 for none of it and 1 for all of it.", "AnimationTree").param("layer", "Aim", "Layer", "The Blend2 or Add2 node in the tree, by its own name.", "blend_layer").param("amount", "1.0", "To", "How much of the layer to mix in: 0 for none, 1 for all of it.", "expression").param("seconds", "0.2", "Over", "How long the fade takes, in seconds.", "expression").param_typed("Node", "target", "self", "On node", "The AnimationTree to blend. Leave it as self when this sheet is the tree.", "scene_node").featured())
	...
	descriptors.append(F.expr("AnimationTimeInState", "Time In State", "get(\"parameters/playback\").get_current_play_position()", CAT, "time in state", "How many seconds the state machine has been playing its current state - the number a recovery window, a charge-up or a progress bar is measured against.", "AnimationTree"))
	...
	descriptors.append(F.trig("OnAnimationStateEntered", "On State Entered", "state_started", CAT, "On state entered", "Runs the moment the state machine enters a state, with the state's name handed to the event. The state machine's own answer to \"the swing has started\" - no polling, no per-frame question.", "AnimationTree"))
	descriptors.append(F.trig("OnAnimationStateLeft", "On State Left", "state_finished", CAT, "On state left", "Runs the moment the state the machine was in finishes, with its name handed to the event - the recovery that begins when the swing ends.", "AnimationTree"))
