...
static func _normalize_params(raw_params: Variant) -> Array[ACEParam]:
	...
	for entry: Variant in raw_params:
		...
		param.description = str(data.get("description", data.get("desc", "")))
		param.desc = str(data.get("desc", param.description))
...
static func _apply_param_aliases(param: ACEParam) -> void:
	...
	if param.initial_value == null:
		param.initial_value = param.initialValue
	if param.initial_value == null:
		param.initial_value = param.default_value
