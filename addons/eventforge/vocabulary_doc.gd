...
static func generate() -> String:
	...
	if not builtin_lines.is_empty():
		...
		lines.append("")
		lines.append("Every action, condition and expression the picker offers with no pack enabled,")
		lines.append("grouped by the module that authors it. Deprecated ones are marked - they still")
		lines.append("compile, but the picker hides them.")
