...
## The one reading of an annotation spec line. Held here rather than written out twice, because the
## provider scanner reads the SAME lines and a split only one of them gets right publishes one
## vocabulary and opens as another - invisibly, since the byte gate only asks whether the compiler
## reproduces what this file read.
...
## the same reading the analyzer gives an annotation, because it is literally the same function. A
## separator inside a string literal, or between the braces of a Dictionary default, is a character
...
static func _split_outside_quotes(text: String, separator: String) -> PackedStringArray:
	return SpecText.split_outside_quotes(text, separator)
