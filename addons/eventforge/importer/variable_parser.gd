...
func _parse_literal(text: String) -> Variant:
	...
	if (value.begins_with("[") and value.ends_with("]")) or (value.begins_with("{") and value.ends_with("}")):
		...
		if parsed != null:
			return parsed
	# The constructor literals for game-value types (Vector2/Color): str_to_var yields the typed value,
	# whose canonical re-emission (sheet_compiler._to_code_literal) the verify-lift gates byte-for-byte.
