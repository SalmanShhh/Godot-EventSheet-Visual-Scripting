...
- `$ComboBoxAddon.buffer_length` inserts the **Buffer Length** entry straight into any expression
- `$ComboBoxAddon.debug_logging` inserts the **Debug Logging** entry straight into any expression

The `$ComboBoxAddon` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("ComboBoxAddon")` chains,
