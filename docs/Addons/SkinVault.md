...
7. [The data-driven path: a SkinCatalogResource asset](#the-data-driven-path-a-skincatalogresource-asset)
...
- `$SkinVaultAddon.enable_pity` inserts the **Enable Pity** entry straight into any expression
- `$SkinVaultAddon.pity_rarity` inserts the **Pity Rarity** entry straight into any expression

The `$SkinVaultAddon` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("SkinVaultAddon")` chains,
