...
- `$TimeSlicerBehavior.frame_budget_ms` inserts the **Frame Budget Ms** entry straight into any expression
- `$TimeSlicerBehavior.max_items_per_frame` inserts the **Max Items Per Frame** entry straight into any expression

The `$TimeSlicerBehavior` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("TimeSlicerBehavior")` chains,
