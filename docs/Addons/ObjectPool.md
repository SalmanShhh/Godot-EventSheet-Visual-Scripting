...
- `$ObjectPoolAddon.spawn(pool_name)` inserts the **Spawn** entry straight into any expression
- `$ObjectPoolAddon.last_spawned()` inserts the **Last Spawned** entry straight into any expression

The `$ObjectPoolAddon` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("ObjectPoolAddon")` chains,
