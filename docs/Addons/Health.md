...
**The difficulty is a field on the row, not a multiplication in front of it.** Take Damage Of Type takes a **Scaled By** name - `damage_taken` on the player's side, `damage_dealt` on the enemy's - and scales the hit by that difficulty factor before anything else touches it, so Last Damage Before Mitigation reads as the hit this game is being played with rather than the number the row was authored with. A factor the difficulty in force has no key for, and a game that chose no difficulty at all, both count as 1, so the field is safe to fill in before any difficulty file mentions it - and blank means the hit is not scaled by anything. Nothing else in this pack knows what a difficulty is: the factors are read from where every script can already see them, so no Settings autoload is required.

**The assist window is counted from the death.** Assists Of lists everyone else who damaged this actor within Assist Seconds of the killing blow - not within Assist Seconds of the moment the row asks - so a results screen read half a minute after the fight still names the same helpers it would have named at the kill. While the actor is alive the window is counted from now, which is what a live assist list means. The hits themselves are not kept for ever: one older than the window, and one from something that has since been freed, are let go the next time a hit lands.
...
| Take Damage Of Type | `amount` (float), `type` (String), `from` (Node), `scaled_by` (String) | Damage that knows what kind it is and who dealt it. Resistance comes off as a percentage, then armour as flat points (never below `minimum_damage`), then a critical multiplies what got through, and the pools and health of Take Damage finish the job. The report is written before On Damaged fires. `scaled_by` names a difficulty factor to scale the hit by first, and is safe to leave blank. |
...
- **Plain Take Damage is a number and nothing else.** It says how much, not of what and not by whom, and it clears nothing: after a typed hit, Damage Type Is, Last Hit Was A Crit and Last Damage Dealt still describe that typed hit, and a killing blow dealt by plain Take Damage leaves Killer Of naming whoever hit the actor last. Take Damage From clears the kind and the critical and names the source; Take Damage Of Type writes the whole report. Use one of those two wherever the damage has an author - a hazard node is an author, so lava can pass itself.
