...
- `$LOS3DBehavior.collision_mask` inserts the **Collision Mask** entry straight into any expression
- `$LOS3DBehavior.cone_of_view_degrees` inserts the **Cone Of View Degrees** entry straight into any expression

The `$LOS3DBehavior` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("LOS3DBehavior")` chains,
