...
- `$RotateBehavior.rotate_enabled` inserts the **Rotate Enabled** entry straight into any expression
- `$RotateBehavior.speed` inserts the **Speed** entry straight into any expression

The `$RotateBehavior` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("RotateBehavior")` chains,
