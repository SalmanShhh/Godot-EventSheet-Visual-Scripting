...
| Expression | Current Path Action | What this leg is: `walk`, `jump`, or `fall`. |
...
- **It must be a sibling of PlatformerMovement.** The pathfinder finds the movement pack among
  the host's children (anything with a `move_speed` and a `jump()`), derives jump reach from
  it, and steers it. No movement sibling? It still paths, but nothing moves in auto mode - use
  manual mode and drive yourself.
