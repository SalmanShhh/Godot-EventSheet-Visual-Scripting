...
- `$DragDropBehavior.break_distance` inserts the **Break Distance** entry straight into any expression
- `$DragDropBehavior.enabled` inserts the **Enabled** entry straight into any expression

The `$DragDropBehavior` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("DragDropBehavior")` chains,
