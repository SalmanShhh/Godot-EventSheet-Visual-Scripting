...
- **Reset keeps definitions, Define Milestone rebuilds one.** Reset un-reaches everything and zeroes progress but leaves thresholds and rewards intact - ideal for prestige. Define Milestone re-creates a single record from nothing, which also clears its latch.
