...
- `$HTNAgent.auto_replan_on_fail` inserts the **Auto Replan On Fail** entry straight into any expression
- `$HTNAgent.root_task` inserts the **Root Task** entry straight into any expression

The `$HTNAgent` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("HTNAgent")` chains,
