...
**The assist radius is measured in this pack's own units.** One options slider feeds both twins, and the radius it carries is read here in metres - the 3D twin reads the same number in pixels. A radius that feels right in a 2D game is therefore not the radius a 3D game wants, so a project with both gives them each their own setting rather than one slider for the pair.
