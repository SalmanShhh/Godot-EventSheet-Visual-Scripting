...
- `$MoveTo3DBehavior.max_speed` inserts the **Max Speed** entry straight into any expression
- `$MoveTo3DBehavior.step_hits_areas` inserts the **Step Hits Areas** entry straight into any expression

The `$MoveTo3DBehavior` token stays selected after insert, so retargeting to your child's actual name is one
keystroke, or a node drag. Attaching this behaviour at runtime instead? Tick **Robust behaviour
lookups** in the dictionary and the same entries insert as `get_node_or_null("MoveTo3DBehavior")` chains,
