...
- **Never write inside `res://` from a Doctor check.** A health check reads and reports; it must not mutate the project. Send any temporary work to `user://`. (Tools that *generate* files are a different job - those legitimately write to `res://`.)
