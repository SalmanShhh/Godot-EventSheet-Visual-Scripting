...
4. **[medium] Autoload id mismatch** - `get_provider_id` `capitalize()` ("Weapon Kit") vs `_autoload_provider_names` `to_pascal_case()` ("WeaponKit"). Reconcile before shipping autoload node-form. Deferred.
...
**Deferred (documented follow-ups):** plain-`var`/property exposure + property-write codegen (the covenant gap); autoload singleton-prefix node-form; the instance-backed **codegen preview** blank (preview reads `metadata.codegen_template`, which is empty for instance-backed - `event_sheet_viewport.gd:3162`); non-primitive-signature method handling.
