...
**Status:** Part I **Phase 1 SHIPPED** (`21bf936` — ACE Studio primitives: verb-kind cards + live picker preview + "Ships as:" strip on `function_dialog.gd`); Part I Phases 2–6 folded into the unified roadmap (§15). Part II **proposed** (second design tournament complete — four in-sheet dimensions [Glance / Flow / Navigate / Commit] designed independently, then ranked, cut, and scored by four verification-first judges; every load-bearing code citation below survived re-verification against the working tree). **Author trigger:** the recurring "isn't this just slower typing / how is this better than Scratch?" critique, and the north-star commitment that *the eventsheets are not done until (1) a stranger's sheet tells you things INSTANTLY, (2) working inside the sheet runs at the speed of thought, and (3) going FULL code-free for a game project is completely possible.* This spec owns the **authoring experience + block-design system** (Part I) **and the in-sheet working experience** (Part II). It cross-references — and does not duplicate:
