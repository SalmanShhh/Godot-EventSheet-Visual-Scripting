...
`tests/custom_block_test.gd`). P2 SHIPPED too (zero-config pack kinds via the addon scan; living proof eventsheet_addons/demo_note_block.gd + the guide chapter). P3: palette integration SHIPPED (Ctrl+P lists every addable kind), and the API is DOGFOODED - `EnumRow` is a registered RESOURCE kind (`handles()`/`emit_lines()`/`summary_for()`, lift returns the resource), so the compiler's enum emission, the importer's enum lift, and the viewport's enum summary all run through the registry byte-identically. The custom-dialog hook SHIPPED (kind.edit; the dock routes every block edit through the registry - enum/signal dialogs dispatch through it), plus code registration via EventForgeBridgeRuntime.register_block_kind for other plugins. SignalRow's declaration contract is on the registry; MatchRow stays out of scope BY DESIGN (it lives inside events - a different seam than between-event blocks). THE API IS COMPLETE: model, emit, lift, render, edit, add surfaces, folder scan, and bridge registration all dispatch through EventSheetBlockRegistry.
