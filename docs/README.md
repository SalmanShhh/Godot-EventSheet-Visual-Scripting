...
- [Visual Effects and Shaders](GUIDE-VISUAL-EFFECTS-AND-SHADERS.md) - turning a shader's dials as rows whose names come out of the `.gdshader` file rather than being typed: the picker shelf per wearing node, the muted `effect.` lead, the four verbs, the shared-material trap and the row that fixes it, the amber note when a shader stops declaring a dial, and the table of what a project that already has shader code opens as.
