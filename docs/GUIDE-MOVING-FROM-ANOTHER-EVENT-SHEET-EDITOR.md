...
| Destroy | **Remove Now** (`queue_free()`), with the timing said on the row: the deletion lands at the END of the frame, so the rows after it still run and the node is still there while they do. `Queue Free` is the same line under its Godot name, and both are unchanged |
| Wait / fade, then destroy | **Remove After Seconds** (`get_tree().create_timer(2.0).timeout.connect(queue_free)` - never blocks, and safe if something else got there first) and **Fade Out Then Remove** (a tween down `modulate:a`, an `await`, then the removal - the event waits, so the row asks again before it removes). **Is Still Here** is the question on its own, beside the shipped Object Still Exists |
| Object count / pick nth instance | A crowd is a plain Godot **group**, and nothing else: **Spawn A Copy Into The Crowd** joins it (`add_to_group(name, true)` - the persistent flag matters), **How Many Alive** is the group's size as an expression, and the cap lives on the row with its policy in the sentence - **Oldest Goes First** or **Unless The Crowd Is Full**. Nothing keeps a list, so nothing can go stale |
| On destroyed | **On Exit Tree** for a node hearing about its own removal, and **On The Last One Removed** for a crowd emptying - the scene tree's own `node_removed` signal, with the question that narrows it to one crowd added underneath as an ordinary, editable condition row |
