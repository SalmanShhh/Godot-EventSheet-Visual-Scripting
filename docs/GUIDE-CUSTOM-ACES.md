...
| `@ace_source("https://...")` (class-level) | Where this pack publishes from. The Addon manager's **Source** button and **Check for updates** have nowhere to look without it; a pack that names none simply says so. (**Update…** does not need it - it reads a .zip you point it at.) |
...
| `options` | Array | A **fixed** dropdown: the user can only pick from these. Entries are either plain strings or `{"key": <inserted>, "label": <shown>}` pairs, so the menu can read "Fill the screen" while the row inserts `fill`. The key is inserted into the call VERBATIM, so for a String parameter the ACE's `codegen_template` has to write the GDScript quotes around that slot - `"$CameraRailBehavior.fly_along({path}, {seconds}, \"{ease}\")"` - or a picked `ease_in` emits a bare identifier the game will not parse. Quoting the key itself is not the fix: the key is the value spliced in, and quotes put there are read off again on the way through. |
