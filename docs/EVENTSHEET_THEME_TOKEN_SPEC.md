...
> Bundled presets: high-contrast, soft-light, Construct3-stacked, designer template, and
> the iconic set — **Dracula, Nord, Gruvbox Dark, Monokai, Solarized Light, Catppuccin
> Mocha** (generated by `tools/build_theme_presets.gd`; rerun it after adding tokens so
> every preset stays complete).
