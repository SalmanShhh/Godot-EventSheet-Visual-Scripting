...
Godot EventSheets (engine codename EventForge): a Godot 4 `@tool` plugin providing a Construct 3-style event sheet editor that compiles sheets to plain, typed GDScript. `addons/eventforge/` is the data model, compiler, importer, and builtin ACE vocabulary; `addons/eventsheet/` is the editor (dock, virtualized viewport, renderer, picker, themes, MCP server, drop-in CSV translations); `eventsheet_addons/` holds the 130 behavior packs (COMPILER OUTPUT, regenerated from `tools/pack_builders/` - builders auto-register by glob, no list to maintain); `demo/showcase/<name>/` holds the playable showcases (also generated - `tools/build_examples.gd`); `AGENTS.md` has the deeper architecture map and standing contracts.
...
- What one hand-written line became: `"$GODOT" --headless --path . --script tools/explain.gd -- res://path/to/file.gd 42` - the row the row builder makes of it (reopen, re-emit, source map), which builder path shaped that row's reading (`shaped`), then which reading layer would have claimed it. `EVENTFORGE_LIFT_ONLY=<entry ids>` reruns one lift-table entry alone, spelled after `EVENTFORGE_TEST_ONLY` so the two compose.
...
- READING GATE (a row's spans are data; a refactor of the reading files must not move one): `"$GODOT" --headless --path . --script tools/reading_dump.gd -- out=user://before.txt`, make the change, dump again to `after.txt`, diff. The population is every builtin descriptor filled with its defaults plus every row of every sheet under `demo/showcase/` and `eventsheet_addons/` (~25,900 cells, ~5 min, byte-stable across runs). `tools/reading_census.gd` reads the same population and reports which builder path shaped each cell, what the two reading files hold, and what the generic assembly would reproduce; `only=builtin` answers in seconds.
...
- How many packs ship, and the two files that quote it: `"$GODOT" --headless --path . --script tools/measure_packs.gd` prints `packs=N builders=N` and names any sentence in README.md / CLAUDE.md that disagrees; add `-- --write` to rewrite those literals. Never hand-type the number - `pack_count_records_test` measures with the same function and fails on drift. A pack is a folder under `eventsheet_addons/` that a builder publishes, so the root runtime scripts (`free_spot.gd`, `pooled_nodes.gd`) and the builder-less `demo_*.gd` do not count; the tool's header spells the whole rule out.
...
- Showcase drift gate (must print `showcases=28 drifted=0`): `"$GODOT" --headless --path . --script tools/build_examples.gd -- --check` - snapshots `demo/showcase/`, runs the ordinary build over it, compares every byte, puts the committed bytes back, names each drifted file, exit 1 on drift. `tests/showcase_drift_test.gd` runs it in the serial tail. A change to a PACK's member order drifts this tree without touching it, which is how five `.tscn` files went stale unnoticed - so regenerate after a pack change, not only after a builder change. A check killed half way leaves the tree REGENERATED (the correct tree); `git status` names it.
...
- Harvest the translation CSVs (appends the owed key to all nine, never deletes; must print
  ...
  gate. The engine errors it prints between `walking the live editor` and `live editor walked` are
  EXPECTED and say so: the advisory half opens real dialogs with no editor around them, derives
  nothing, and the verdict line and exit code are unaffected.
...
- Release ritual: bump `sheet_compiler.gd` `VERSION` + `addons/eventforge/plugin.cfg`, regenerate the compiler golden (`tools/regenerate_demo_golden.gd`, writes `tests/fixtures/compiler_golden_sheet_generated.gd`), finalize the CHANGELOG header + `docs/internal/RELEASE-NOTES-vX.md`, refresh README status/milestones, rewrite the pack counts with `"$GODOT" --headless --path . --script tools/measure_packs.gd -- --write` (never by hand), delete shipped specs from `docs/internal/`, then commit + annotated tag `vX.Y.Z` + `git push --follow-tags`.
